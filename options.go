...
// This file contains all dep2p configuration options (except the defaults,
...
import (
	...
	"github.com/bpfs/dep2p/core/metrics"
	"github.com/bpfs/dep2p/core/network"
	"github.com/bpfs/dep2p/core/peer"
	"github.com/bpfs/dep2p/core/peerstore"
	"github.com/bpfs/dep2p/core/pnet"
	"github.com/bpfs/dep2p/core/protocol"
	"github.com/bpfs/dep2p/core/transport"
	"github.com/bpfs/dep2p/p2p/host/autorelay"
	bhost "github.com/bpfs/dep2p/p2p/host/basic"
	"github.com/bpfs/dep2p/p2p/net/swarm"
	tptu "github.com/bpfs/dep2p/p2p/net/upgrader"
...
// ListenAddrStrings configures dep2p to listen on the given (unparsed)
// addresses.
...
// Security configures dep2p to use the given security transport (or transport
// constructor).
//
// Name is the protocol name.
//
// The transport can be a constructed security.Transport or a function taking
// any subset of this dep2p node's:
// * Public key
// * Private key
// * Peer ID
// * Host
...
// Transport configures dep2p to use the given transport (or transport
// constructor).
//
...
// This option only configures dep2p to accept inbound connections from relays
...
// EnableRelayService configures dep2p to run a circuit v2 relay,
...
// EnableAutoRelayWithStaticRelays configures dep2p to enable the AutoRelay subsystem using
...
// EnableAutoRelayWithPeerSource configures dep2p to enable the AutoRelay
...
// EnableNATService configures dep2p to provide a service to peers for determining
...
// ConnectionGater configures dep2p to use the given ConnectionGater
...
// it is recommended to set limits for dep2p protocol by calling SetDefaultServiceLimits.
...
// NATPortMap configures dep2p to use the default NATManager. The default
// NATManager will attempt to open a port in your network's firewall using UPnP.
func NATPortMap() Option {
	return NATManager(bhost.NewNATManager)
}

// NATManager will configure dep2p to use the requested NATManager. This
// function should be passed a NATManager *constructor* that takes a dep2p Network.
...
//
// This will both clear any configured listen addrs and prevent dep2p from
...
//
// This will both clear any configured transports (specified in prior dep2p
// options) and prevent dep2p from applying the default transports.
...
// DialRanker configures dep2p to use d as the dial ranker. To enable smart
