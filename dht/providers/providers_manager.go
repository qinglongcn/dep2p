...
import (
	...
	peerstoreImpl "github.com/bpfs/dep2p/p2p/host/peerstore"
	...
	go func() {
		...
		defer func() {
			...
			if err := pm.dstore.Flush(context.Background()); err != nil {
				logrus.Error("failed to flush datastore: ", err)
			}
		}()
		...
		for {
			...
			case np := <-pm.newprovs:
				...
				if err != nil {
					logrus.Error("error adding new providers: ", err)
					continue
				}
				if gcSkip != nil {
			...
			case gp := <-pm.getprovs:
				...
				if err != nil && err != ds.ErrNotFound {
					logrus.Error("error reading providers: ", err)
				}
			...
			case res, ok := <-gcQueryRes:
				if !ok {
					if err := gcQuery.Close(); err != nil {
						logrus.Error("failed to close provider GC query: ", err)
					}
				...
				if res.Error != nil {
					logrus.Error("got error from GC query: ", res.Error)
					continue
				}
				...
				case err != nil:
					...
					logrus.Error("parsing providers record from disk: ", err)
					fallthrough
				case gcTime.Sub(t) > ProvideValidity:
					// or expired
					err = pm.dstore.Delete(pm.ctx, ds.RawKey(res.Key))
					if err != nil && err != ds.ErrNotFound {
						logrus.Error("failed to remove provider record from disk: ", err)
					}
				}
			...
			case gcTime = <-gcTimer.C:
				...
				if err != nil {
					logrus.Error("provider record GC query failed: ", err)
					continue
				}
				gcQuery = q
				gcQueryRes = q.Next()
...
func loadProviderSet(ctx context.Context, dstore ds.Datastore, k []byte) (*providerSet, error) {
	...
	for {
		...
		if e.Error != nil {
			logrus.Error("got an error: ", e.Error)
			continue
		}
		...
		if err != nil {
			logrus.Error("base32 decoding error: ", err)
			err = dstore.Delete(ctx, ds.RawKey(e.Key))
			if err != nil && err != ds.ErrNotFound {
				logrus.Error("failed to remove provider record from disk: ", err)
			}
