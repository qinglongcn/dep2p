...
func (m *messageSenderImpl) SendRequest(ctx context.Context, p peer.ID, pmes *pb.Message) (*pb.Message, error) {
	...
	if err != nil {
		...
		logrus.Debug("request failed to open message sender", "error", err, "to", p)
		return nil, err
	}
	...
	if err != nil {
		...
		logrus.Debug("request failed", "error", err, "to", p)
		return nil, err
	}
...
func (m *messageSenderImpl) SendMessage(ctx context.Context, p peer.ID, pmes *pb.Message) error {
	...
	if err != nil {
		...
		logrus.Debug("message failed to open message sender", "error", err, "to", p)
		return err
	}
	...
	if err := ms.SendMessage(ctx, pmes); err != nil {
		...
		logrus.Debug("message failed", "error", err, "to", p)
		return err
	}
