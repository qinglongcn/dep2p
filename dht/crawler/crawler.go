...
import (
	...
	"github.com/bpfs/dep2p/util/msgio/protoio"

	pb "github.com/bpfs/dep2p/dht/pb"
	kbucket "github.com/bpfs/dep2p/util/kbucket"
)
...
func (c *DefaultCrawler) Run(ctx context.Context, startingPeers []*peer.AddrInfo, handleSuccess HandleQueryResult, handleFail HandleQueryFail) {
	...
	if numSkipped > 0 {
		logrus.Infof("%d starting peers were skipped due to lack of addresses. Starting crawl with %d peers", numSkipped, len(toDial))
	}
	...
	for len(toDial) > 0 || outstanding > 0 {
		...
		case res := <-results:
			if len(res.data) > 0 {
				logrus.Debugf("peer %v had %d peers", res.peer, len(res.data))
		...
		case jobCh <- nextPeerID:
			...
			numQueried++
			toDial = toDial[1:]
			logrus.Debugf("starting %d out of %d", numQueried, len(peersSeen))
		}
	}
}
...
func (c *DefaultCrawler) queryPeer(ctx context.Context, nextPeer peer.ID) *queryResult {
	...
	if err != nil {
		logrus.Errorf("error creating rt for peer %v : %v", nextPeer, err)
	...
	if err != nil {
		logrus.Debugf("could not connect to peer %v: %v", nextPeer, err)
		return &queryResult{nextPeer, nil, err}
	}
	...
	for cpl := 0; cpl <= 15; cpl++ {
		...
		if err != nil {
			logrus.Debugf("error finding data on peer %v with cpl %d : %v", nextPeer, cpl, err)
			retErr = err
			break
		}
