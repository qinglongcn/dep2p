...
import (
	...
	"github.com/bpfs/dep2p/dht/qpeerset"
...
// This file implements the Routing interface for the DeP2PDHT struct.

// Basic Put/Get

// PutValue adds value corresponding to given Key.
// This is the top level "Store" operation of the DHT
func (dht *DeP2PDHT) PutValue(ctx context.Context, key string, value []byte, opts ...routing.Option) (err error) {
	...
	for _, p := range peers {
		...
		go func(p peer.ID) {
			...
			if err != nil {
				logrus.Debugf("failed putting value to peer: %s", err)
			}
		}(p)
	}
	wg.Wait()
...
func (dht *DeP2PDHT) GetValue(ctx context.Context, key string, opts ...routing.Option) (result []byte, err error) {
...
func (dht *DeP2PDHT) SearchValue(ctx context.Context, key string, opts ...routing.Option) (ch <-chan []byte, err error) {
...
func (dht *DeP2PDHT) searchValueQuorum(ctx context.Context, key string, valCh <-chan recvdVal, stopCh chan struct{},
...
func (dht *DeP2PDHT) processValues(ctx context.Context, key string, vals <-chan recvdVal,
...
func (dht *DeP2PDHT) updatePeerValues(ctx context.Context, key string, val []byte, peers []peer.ID) {
...
func (dht *DeP2PDHT) getValues(ctx context.Context, key string, stopQuery chan struct{}) (<-chan recvdVal, <-chan *lookupWithFollowupResult) {
...
func (dht *DeP2PDHT) Provide(ctx context.Context, key cid.Cid, brdcst bool) (err error) {
	...
	if dht.enableOptProv {
		...
		if errors.Is(err, netsize.ErrNotEnoughData) {
			logrus.Debugln("not enough data for optimistic provide taking classic approach")
...
func (dht *DeP2PDHT) classicProvide(ctx context.Context, keyMH multihash.Multihash) error {
	...
	for _, p := range peers {
		...
		go func(p peer.ID) {
			defer wg.Done()
			logrus.Debugf("putProvider(%s, %s)", internal.LoggableProviderRecordBytes(keyMH), p)
...
func (dht *DeP2PDHT) FindProviders(ctx context.Context, c cid.Cid) ([]peer.AddrInfo, error) {
	if !dht.enableProviders {
		return nil, routing.ErrNotSupported
	} else if !c.Defined() {
...
// from progressing.
func (dht *DeP2PDHT) FindProvidersAsync(ctx context.Context, key cid.Cid, count int) (ch <-chan peer.AddrInfo) {
...
func (dht *DeP2PDHT) findProvidersAsyncRoutine(ctx context.Context, key multihash.Multihash, count int, peerOut chan peer.AddrInfo) {
	...
	ctx, span := internal.StartSpan(ctx, "DeP2PDHT.FindProvidersAsyncRoutine")
	defer span.End()
...
func (dht *DeP2PDHT) FindPeer(ctx context.Context, id peer.ID) (pi peer.AddrInfo, err error) {
