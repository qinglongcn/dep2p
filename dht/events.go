...
import (
	...
	"github.com/bpfs/dep2p/util/kbucket"
)
...
// PeerKadID contains a dep2p Peer ID and a binary Kademlia ID.
type PeerKadID struct {
	Peer peer.ID
	Kad  kbucket.ID
}

// NewPeerKadID creates a PeerKadID from a dep2p Peer ID.
...
// NewPeerKadIDSlice creates a slice of PeerKadID from the passed slice of dep2p Peer IDs.
...
// dep2p Peer ID to a PeerKadID and the string Kademlia key to a KeyKadID.
func NewLookupEvent(
	node peer.ID,
	id uuid.UUID,
	key string,
