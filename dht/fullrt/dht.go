...
import (
	...
	"github.com/bpfs/dep2p/core/peerstore"
	"github.com/bpfs/dep2p/core/protocol"
	"github.com/bpfs/dep2p/core/routing"
	swarm "github.com/bpfs/dep2p/p2p/net/swarm"
	"github.com/bpfs/dep2p/util/routinghelpers/tracing"

	"github.com/gogo/protobuf/proto"
	ds "github.com/ipfs/go-datastore"
	dssync "github.com/ipfs/go-datastore/sync"
	u "github.com/bpfs/dep2p/util"
	"github.com/bpfs/dep2p/util/cid"

	kaddht "github.com/bpfs/dep2p/dht"
	"github.com/bpfs/dep2p/dht/crawler"
	"github.com/bpfs/dep2p/dht/internal"
	internalConfig "github.com/bpfs/dep2p/dht/internal/config"
	"github.com/bpfs/dep2p/dht/internal/net"
	dht_pb "github.com/bpfs/dep2p/dht/pb"
	"github.com/bpfs/dep2p/dht/providers"
	kb "github.com/bpfs/dep2p/util/kbucket"

	"github.com/bpfs/dep2p/util/record"
	recpb "github.com/bpfs/dep2p/util/record/pb"

	"github.com/bpfs/dep2p/util/xor/kademlia"
	kadkey "github.com/bpfs/dep2p/util/xor/key"
	"github.com/bpfs/dep2p/util/xor/trie"
	"go.opentelemetry.io/otel/attribute"
	"go.opentelemetry.io/otel/trace"
)

const tracer = tracing.Tracer("go-dep2p-kad-dht/fullrt")
...
// For example, the protocol /dep2p/kad/1.0.0 has the prefix /dep2p.
//
// FullRT is an experimental DHT client that is under development. Expect breaking changes to occur in this client
// until it stabilizes.
//
	...
	for {
		...
		dht.crawler.Run(ctx, addrs,
			func(p peer.ID, rtPeers []*peer.AddrInfo) {
				...
				if len(addrs) == 0 {
					logrus.Debugf("no connections to %v after successful query. keeping addresses from the peerstore", p)
				...
				if ok {
					for _, transportErr := range dialErr.DialErrors {
						if errors.Is(transportErr.Cause, network.ErrResourceLimitExceeded) {
							limitErrOnce.Do(func() { logrus.Errorf(rtRefreshLimitsMsg) })
						}
					}
				}
...
func (dht *FullRT) GetClosestPeers(ctx context.Context, key string) ([]peer.ID, error) {
	...
	for _, k := range closestKeys {
		...
		if !ok {
			logrus.Errorf("key not found in map")
		}
		dht.kMapLk.RUnlock()
		dht.peerAddrsLk.RLock()
...
func (dht *FullRT) PutValue(ctx context.Context, key string, value []byte, opts ...routing.Option) (err error) {
	...
	logrus.Debug("putting value", "key", internal.LoggableRecordKeyString(key))
...
func (dht *FullRT) GetValue(ctx context.Context, key string, opts ...routing.Option) (result []byte, err error) {
	...
	logrus.Debugf("GetValue %v %x", internal.LoggableRecordKeyString(key), best)
	return best, nil
}
	...
	for {
		...
		case v, ok := <-vals:
			...
			if best != nil {
				...
				if err != nil {
					logrus.Warn("failed to select best value", "key", internal.LoggableRecordKeyString(key), "error", err)
					continue
				}
				if sel != 1 {
					aborted = newVal(ctx, v, false)
	...
	for _, p := range peers {
		go func(p peer.ID) {
			...
			if p == dht.h.ID() {
				...
				if err != nil {
					logrus.Error("Error correcting local dht entry:", err)
				}
				return
			}
			ctx, cancel := context.WithTimeout(ctx, time.Second*5)
			...
			if err != nil {
				logrus.Debug("Error correcting DHT entry: ", err)
			}
		}(p)
	}
}
...
func (dht *FullRT) getValues(ctx context.Context, key string, stopQuery chan struct{}) (<-chan RecvdVal, <-chan *lookupWithFollowupResult) {
	...
	logrus.Debug("finding value", "key", internal.LoggableRecordKeyString(key))
	...
	go func() {
		...
		queryFn := func(ctx context.Context, p peer.ID) error {
			...
			val := rec.GetValue()
			if val == nil {
				logrus.Debug("received a nil record value")
				return nil
			}
			if err := dht.Validator.Validate(key, val); err != nil {
				// make sure record is valid
				logrus.Debug("received invalid record (discarded)", "error", err)
				return nil
			}
...
func (dht *FullRT) Provide(ctx context.Context, key cid.Cid, brdcst bool) (err error) {
	...
	keyMH := key.Hash()
	logrus.Debug("providing", "cid", key, "mh", internal.LoggableProviderRecordBytes(keyMH))
...
func (dht *FullRT) bulkMessageSend(ctx context.Context, keys []peer.ID, fn func(ctx context.Context, target, k peer.ID) error, isProvRec bool) error {
	...
	logrus.Infof("bulk send: number of keys %d, unique %d", len(keys), len(keySuccesses))
	...
	for i := 0; i < dht.bulkSendParallelism; i++ {
		go func() {
			defer wg.Done()
			defer logrus.Debugf("bulk send goroutine done")
	...
	for _, g := range keyGroups {
		...
		logrus.Debugf("bulk send: %d peers for group size %d", len(keysPerPeer), len(g))
	...
	keyloop:
		...
		sendsSoFar += len(g)
		logrus.Infof("bulk sending: %.1f%% done - %d/%d done", 100*float64(sendsSoFar)/float64(len(keySuccesses)), sendsSoFar, len(keySuccesses))
	}

	close(workCh)

	logrus.Debugf("bulk send complete, waiting on goroutines to close")
	...
	if numSendsSuccessful == 0 {
		logrus.Infof("bulk send failed")
		return fmt.Errorf("failed to complete bulk sending")
	}

	logrus.Infof("bulk send complete: %d keys, %d unique, %d successful, %d skipped peers, %d fails",
		len(keys), len(keySuccesses), numSendsSuccessful, numSkipped, numFails)

	logrus.Infof("bulk send summary: successHist %v, failHist %v", successHist, failHist)
...
func (dht *FullRT) FindProvidersAsync(ctx context.Context, key cid.Cid, count int) (ch <-chan peer.AddrInfo) {
	...
	logrus.Debug("finding providers", "cid", key, "mh", internal.LoggableProviderRecordBytes(keyMH))
...
func (dht *FullRT) findProvidersAsyncRoutine(ctx context.Context, key multihash.Multihash, count int, peerOut chan peer.AddrInfo) {
	...
	fn := func(ctx context.Context, p peer.ID) error {
		...
		logrus.Debugf("%d provider entries", len(provs))

		// Add unique providers from request, up to 'count'
		for _, prov := range provs {
			dht.maybeAddAddrs(prov.ID, prov.Addrs, peerstore.TempAddrTTL)
			logrus.Debugf("got provider: %s", prov)
			if psTryAdd(prov.ID) {
				logrus.Debugf("using provider: %s", prov)
				select {
				case peerOut <- *prov:
					span.AddEvent("found provider", trace.WithAttributes(
				...
				case <-ctx.Done():
					logrus.Debug("context timed out sending more providers")
					return ctx.Err()
				}
			}
			if !findAll && psSize() >= count {
				logrus.Debugf("got enough providers (%d/%d)", psSize(), count)
				cancelquery()
				return nil
			}
		}
		...
		logrus.Debugf("got closer peers: %d %s", len(closest), closest)
...
func (dht *FullRT) FindPeer(ctx context.Context, id peer.ID) (pi peer.AddrInfo, err error) {
	...
	logrus.Debug("finding peer", "peer", id)
...
func (dht *FullRT) getLocal(ctx context.Context, key string) (*recpb.Record, error) {
	logrus.Debug("finding value in datastore", "key", internal.LoggableRecordKeyString(key))

	rec, err := dht.getRecordFromDatastore(ctx, mkDsKey(key))
	if err != nil {
		logrus.Warn("get local failed", "key", internal.LoggableRecordKeyString(key), "error", err)
		return nil, err
	}

	// Double check the key. Can't hurt.
	if rec != nil && string(rec.GetKey()) != key {
		logrus.Error("BUG: found a DHT record that didn't match it's key", "expected", internal.LoggableRecordKeyString(key), "got", rec.GetKey())
		return nil, nil
...
func (dht *FullRT) putLocal(ctx context.Context, key string, rec *recpb.Record) error {
	...
	if err != nil {
		logrus.Warn("failed to put marshal record for local put", "error", err, "key", internal.LoggableRecordKeyString(key))
		return err
	}
...
func (dht *FullRT) getRecordFromDatastore(ctx context.Context, dskey ds.Key) (*recpb.Record, error) {
	...
	if err != nil {
		logrus.Error("error retrieving record from datastore", "key", dskey, "error", err)
		return nil, err
	}
	...
	if err != nil {
		...
		logrus.Error("failed to unmarshal record from datastore", "key", dskey, "error", err)
		return nil, nil
	}
	...
	if err != nil {
		...
		logrus.Debug("local record verify failed", "key", rec.GetKey(), "error", err)
		return nil, nil
	}
