...
func (dht *DeP2PDHT) newOptimisticState(ctx context.Context, key string) (*optimisticState, error) {
...
func (dht *DeP2PDHT) optimisticProvide(outerCtx context.Context, keyMH multihash.Multihash) error {
	key := string(keyMH)
	...
	if err = dht.nsEstimator.Track(key, lookupRes.closest); err != nil {
		logrus.Warnf("network size estimator track peers: %s", err)
	}
