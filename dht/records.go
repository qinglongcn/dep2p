...
func (dht *DeP2PDHT) GetPublicKey(ctx context.Context, p peer.ID) (ci.PubKey, error) {
	ctx, span := internal.StartSpan(ctx, "DeP2PDHT.GetPublicKey", trace.WithAttributes(attribute.Stringer("PeerID", p)))
	defer span.End()
	...
	logrus.Debugf("getPublicKey for: %s", p)
	...
	for i := 0; i < 2; i++ {
		...
		if r.err == nil {
			...
			if err != nil {
				logrus.Error("failed to add public key to peerstore", "peer", p)
			}
			return r.pubk, nil
		}
		err = r.err
...
func (dht *DeP2PDHT) getPublicKeyFromDHT(ctx context.Context, p peer.ID) (ci.PubKey, error) {
	...
	if err != nil {
		logrus.Errorf("Could not unmarshal public key retrieved from DHT for %v", p)
		return nil, err
	}

	// Note: No need to check that public key hash matches peer ID
	// because this is done by GetValues()
	logrus.Debugf("Got public key for %s from DHT", p)
	return pubk, nil
}

func (dht *DeP2PDHT) getPublicKeyFromNode(ctx context.Context, p peer.ID) (ci.PubKey, error) {
	...
	if err != nil {
		logrus.Errorf("Could not unmarshal public key for %v", p)
		return nil, err
	}
	...
	if err != nil {
		logrus.Errorf("Could not extract peer id from public key for %v", p)
		return nil, err
	}
	if id != p {
	...
	logrus.Debugf("Got public key from node %v itself", p)
	return pubk, nil
}
