...
const tracer = tracing.Tracer("go-dep2p-kad-dht")
const dhtName = "DeP2PDHT"

var (
	// baseLogger = logrus.Desugar()
...
// DeP2PDHT is an implementation of Kademlia with S/Kademlia modifications.
...
// Assert that DeP2P assumptions about interfaces aren't broken. These aren't a
// guarantee, but we can use them to aid refactoring.
var (
	_ routing.ContentRouting = (*DeP2PDHT)(nil)
	_ routing.Routing        = (*DeP2PDHT)(nil)
	_ routing.PeerRouting    = (*DeP2PDHT)(nil)
	_ routing.PubKeyFetcher  = (*DeP2PDHT)(nil)
	_ routing.ValueStore     = (*DeP2PDHT)(nil)
)
...
// DeP2PDHT's initialized with this function will respond to DHT requests,
// whereas DeP2PDHT's initialized with NewDHTClient will not.
func NewDHT(ctx context.Context, h host.Host, dstore ds.Batching) *DeP2PDHT {
...
// host. DeP2PDHT clients initialized with this function will not respond to DHT
// requests. If you need a peer to respond to DHT requests, use NewDHT instead.
func NewDHTClient(ctx context.Context, h host.Host, dstore ds.Batching) *DeP2PDHT {
...
func (dht *DeP2PDHT) ProviderStore() providers.ProviderStore {
	return dht.providerStore
}

// GetRoutingTableDiversityStats returns the diversity stats for the Routing Table.
func (dht *DeP2PDHT) GetRoutingTableDiversityStats() []peerdiversity.CplDiversityStats {
...
func (dht *DeP2PDHT) fixLowPeers() {
	...
	// See https://github.com/bpfs/dep2p/dht/issues/387.
	if dht.routingTable.Size() == 0 && dht.bootstrapPeers != nil {
		...
		for _, i := range rand.Perm(len(bootstrapPeers)) {
			...
			if err == nil {
				...
				logrus.Warn("failed to bootstrap", "peer", ai.ID, "error", err)
			}
...
// SHOULD be removed once https://github.com/bpfs/dep2pissues/800 goes in.
...
func (dht *DeP2PDHT) getLocal(ctx context.Context, key string) (*recpb.Record, error) {
...
func (dht *DeP2PDHT) putLocal(ctx context.Context, key string, rec *recpb.Record) error {
...
func (dht *DeP2PDHT) peerFound(p peer.ID) {
	...
	if err != nil {
		logrus.Error("failed to validate if peer is a DHT peer", "peer", p, "error", err)
	} else if b {
...
func (dht *DeP2PDHT) FindLocal(ctx context.Context, id peer.ID) peer.AddrInfo {
	_, span := internal.StartSpan(ctx, "DeP2PDHT.FindLocal", trace.WithAttributes(attribute.Stringer("PeerID", id)))
	defer span.End()
...
func (dht *DeP2PDHT) betterPeersToQuery(pmes *pb.Message, from peer.ID, count int) []peer.ID {
	...
	// no node? nil
	if closer == nil {
		logrus.Info("no closer peers to send", from)
		return nil
	}
	...
	for _, clp := range closer {
		...
		// == to self? thats bad
		if clp == dht.self {
			logrus.Error("BUG betterPeersToQuery: attempted to return self! this shouldn't happen...")
			return nil
		}
...
// moveToServerMode advertises (via dep2p identify updates) that we are able to respond to DHT queries and sets the appropriate stream handlers.
...
// moveToClientMode stops advertising (and rescinds advertisements via dep2p identify updates) that we are able to
...
// Host returns the dep2p host this DHT is operating with.
...
func (dht *DeP2PDHT) NetworkSize() (int32, error) {
...
func (dht *DeP2PDHT) filterAddrs(addrs []ma.Multiaddr) []ma.Multiaddr {
