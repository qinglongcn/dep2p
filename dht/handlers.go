...
import (
	...
	pstore "github.com/bpfs/dep2p/p2p/host/peerstore"
...
func (dht *DeP2PDHT) handleGetValue(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, err error) {
	...
	if len(closer) > 0 {
		...
		for _, pi := range closerinfos {
			logrus.Debugf("handleGetValue returning closer peer: '%s'", pi.ID)
			if len(pi.Addrs) < 1 {
				logrus.Warn("no addresses on peer being sent",
					"local", dht.self,
					"to", p,
					"sending", pi.ID,
				)
...
func (dht *DeP2PDHT) checkLocalDatastore(ctx context.Context, k []byte) (*recpb.Record, error) {
	logrus.Debugf("%s handleGetValue looking into ds", dht.self)
	dskey := convertToDsKey(k)
	buf, err := dht.datastore.Get(ctx, dskey)
	logrus.Debugf("%s handleGetValue looking into ds GOT %v", dht.self, buf)
	...
	logrus.Debugf("%s handleGetValue success!", dht.self)

	rec := new(recpb.Record)
	err = proto.Unmarshal(buf, rec)
	if err != nil {
		logrus.Debug("failed to unmarshal DHT record from datastore")
		return nil, err
	}
	...
	if err != nil {
		logrus.Info("either no receive time set on record, or it was invalid: ", err)
		recordIsBad = true
	}

	if time.Since(recvtime) > dht.maxRecordAge {
		logrus.Debug("old record found, tossing.")
		recordIsBad = true
	}
	...
	if recordIsBad {
		...
		if err != nil {
			logrus.Error("Failed to delete bad record from datastore: ", err)
		}
...
func (dht *DeP2PDHT) handlePutValue(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, err error) {
	...
	rec := pmes.GetRecord()
	if rec == nil {
		logrus.Debug("got nil record from", "from", p)
	...
	if err = dht.Validator.Validate(string(rec.GetKey()), rec.GetValue()); err != nil {
		logrus.Info("bad dht record in PUT", "from", p, "key", internal.LoggableRecordKeyBytes(rec.GetKey()), "error", err)
		return nil, err
	}
	...
	if existing != nil {
		...
		if err != nil {
			logrus.Warn("dht record passed validation but failed select", "from", p, "key", internal.LoggableRecordKeyBytes(rec.GetKey()), "error", err)
			return nil, err
		}
		if i != 0 {
			logrus.Info("DHT record in PUT older than existing record (ignoring)", "peer", p, "key", internal.LoggableRecordKeyBytes(rec.GetKey()))
...
func (dht *DeP2PDHT) getRecordFromDatastore(ctx context.Context, dskey ds.Key) (*recpb.Record, error) {
	buf, err := dht.datastore.Get(ctx, dskey)
	if err == ds.ErrNotFound {
		return nil, nil
	}
...
func (dht *DeP2PDHT) handleGetProviders(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	key := pmes.GetKey()
	if len(key) > 80 {
...
func (dht *DeP2PDHT) handleAddProvider(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	key := pmes.GetKey()
	if len(key) > 80 {
	...
	logrus.Debug("adding provider", "from", p, "key", internal.LoggableProviderRecordBytes(key))
	...
	for _, pi := range pinfos {
		if pi.ID != p {
			...
			logrus.Debug("received provider from wrong peer", "from", p, "peer", pi.ID)
			continue
		}

		if len(pi.Addrs) < 1 {
			logrus.Debug("no valid addresses for provider", "from", p)
			continue
		}
