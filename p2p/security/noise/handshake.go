...
import (
	...
	"github.com/bpfs/dep2p/util/sha256"
...
// a noise-dep2p session. It blocks until the handshake completes or fails.
...
// generateHandshakePayload creates a dep2p handshake payload with a
...
func (s *secureSession) generateHandshakePayload(localStatic noise.DHKey, ext *pb.NoiseExtensions) ([]byte, error) {
	...
	// our dep2p secret key.
	localKeyRaw, err := crypto.MarshalPublicKey(s.LocalPublicKey())
	if err != nil {
		return nil, fmt.Errorf("error serializing dep2p identity key: %w", err)
	}
...
func (s *secureSession) handleRemoteHandshakePayload(payload []byte, remoteStatic []byte) (*pb.NoiseExtensions, error) {
	...
	// unpack remote peer's public dep2p key
	...
	// verify payload is signed by asserted remote dep2p key.
