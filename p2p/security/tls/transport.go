		...
		for _, proto := range info.SupportedProtos {
			for _, m := range muxers {
				if m == proto {
					...
					// There's no need to add the "dep2p" entry here.
...
func (t *Transport) handshake(ctx context.Context, tlsConn *tls.Conn, keyCh <-chan ci.PubKey) (_sconn sec.SecureConn, err error) {
	...
	if remotePubKey == nil {
		return nil, errors.New("dep2p tls BUG: expected remote pub key to be set")
	}
...
func (t *Transport) setupConn(tlsConn *tls.Conn, remotePubKey ci.PubKey) (sec.SecureConn, error) {
	...
	// The special ALPN extension value "dep2p" is used by dep2p versions
