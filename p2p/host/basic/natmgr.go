...
import (
	...
	inat "github.com/bpfs/dep2p/p2p/net/nat"
...
func (nmgr *natManager) background(ctx context.Context) {
	...
	if err != nil {
		logrus.Info("DiscoverNAT error:", err)
		return
	}
...
func (nmgr *natManager) doSync() {
	...
	for _, e := range newAddresses {
		if err := nmgr.nat.AddMapping(nmgr.ctx, e.protocol, e.port); err != nil {
			logrus.Errorf("failed to port-map %s port %d: %s", e.protocol, e.port, err)
		}
		nmgr.tracked[e] = false
	}
}
...
func (nmgr *natManager) GetMapping(addr ma.Multiaddr) ma.Multiaddr {
	...
	if err != nil {
		logrus.Errorf("error parsing net multiaddr %q: %s", transport, err)
		return nil
	}
	...
	if err != nil {
		logrus.Errorf("mapped addr can't be turned into a multiaddr %q: %s", mappedAddr, err)
		return nil
	}
