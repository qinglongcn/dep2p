...
import (
	...
	"github.com/bpfs/dep2p/core/connmgr"
	...
	"github.com/bpfs/dep2p/p2p/host/autonat"
	"github.com/bpfs/dep2p/p2p/host/eventbus"
	"github.com/bpfs/dep2p/p2p/host/pstoremanager"
	"github.com/bpfs/dep2p/p2p/host/relaysvc"
	relayv2 "github.com/bpfs/dep2p/p2p/protocol/circuitv2/relay"
	"github.com/bpfs/dep2p/p2p/protocol/holepunch"
	"github.com/bpfs/dep2p/p2p/protocol/identify"
	"github.com/bpfs/dep2p/p2p/protocol/ping"
	webtransport "github.com/bpfs/dep2p/p2p/transport/webtransport"
...
type HostOpts struct {
	...
	// ConnManager is a dep2p connection manager
...
func (h *BasicHost) updateLocalIpAddr() {
	...
	if r, err := netroute.New(); err != nil {
		logrus.Debug("failed to build Router for kernel's routing table", "error", err)
	} else {
		if _, _, localIPv4, err := r.Route(net.IPv4zero); err != nil {
			logrus.Debug("failed to fetch local IPv4 address", "error", err)
		...
		if _, _, localIPv6, err := r.Route(net.IPv6unspecified); err != nil {
			logrus.Debug("failed to fetch local IPv6 address", "error", err)
	...
	if err != nil {
		...
		logrus.Error("failed to resolve local interface addresses", "error", err)
...
func (h *BasicHost) newStreamHandler(s network.Stream) {
	...
	if h.negtimeout > 0 {
		if err := s.SetDeadline(time.Now().Add(h.negtimeout)); err != nil {
			logrus.Debug("setting stream deadline: ", err)
			s.Reset()
			return
		}
	}
	...
	if err != nil {
		if err == io.EOF {
			logrus.Debugf("protocol EOF: %s (took %s)", s.Conn().RemotePeer(), took)
		} else {
			logrus.Debugf("protocol mux failed: %s (took %s)", err, took)
		}
		s.Reset()
		return
	}
	...
	if h.negtimeout > 0 {
		if err := s.SetDeadline(time.Time{}); err != nil {
			logrus.Debugf("resetting stream deadline: %s", err)
			s.Reset()
			return
		}
	}
	...
	if err := s.SetProtocol(protoID); err != nil {
		logrus.Debugf("error setting stream protocol: %s", err)
		s.Reset()
		return
	}

	logrus.Debugf("negotiated: %s (took %s)", protoID, took)
...
func (h *BasicHost) background() {
	...
	emitAddrChange := func(currentAddrs []ma.Multiaddr, lastAddrs []ma.Multiaddr) {
		...
		if !h.disableSignedPeerRecord {
			...
			if err != nil {
				logrus.Errorf("error creating a signed peer record from the set of current addresses, err=%s", err)
				return
			}
			...
			if _, err := h.caBook.ConsumePeerRecord(sr, peerstore.PermanentAddrTTL); err != nil {
				logrus.Errorf("failed to persist signed peer record in peer store, err=%s", err)
				return
			}
		}
		...
		if err := h.emitters.evtLocalAddrsUpdated.Emit(*changeEvt); err != nil {
			logrus.Warnf("error emitting event for updated addrs: %s", err)
		}
	}
...
func (h *BasicHost) dialPeer(ctx context.Context, p peer.ID) error {
	logrus.Debugf("host %s dialing %s", h.ID(), p)
	...
	logrus.Debugf("host %s finished dialing %s", h.ID(), p)
	return nil
}
...
func (h *BasicHost) Addrs() []ma.Multiaddr {
	...
	for i, addr := range addrs {
		if ok, n := webtransport.IsWebtransportMultiaddr(addr); ok && n == 0 {
			...
			if !added {
				logrus.Debug("Couldn't add certhashes to webtransport multiaddr because we aren't listening on webtransport")
				continue
			}
...
func (h *BasicHost) AllAddrs() []ma.Multiaddr {
	...
	if resolved, err := manet.ResolveUnspecifiedAddresses(listenAddrs, filteredIfaceAddrs); err != nil {
		...
		logrus.Debug("failed to resolve listen addrs", "error", err)
	} else {
	...
	if h.natmgr != nil && h.natmgr.HasDiscoveredNAT() {
		...
		for _, listen := range listenAddrs {
			...
			if !manet.IsIPUnspecified(extMaddr) {
				...
				logrus.Warn("NAT device reported an unspecified IP as it's external address")
			}
...
func inferWebtransportAddrsFromQuic(in []ma.Multiaddr) []ma.Multiaddr {
	...
	for _, addr := range in {
		isWebtransport, numCertHashes := webtransport.IsWebtransportMultiaddr(addr)
		if isWebtransport {
