...
func (r *AutoRelay) background() {
	...
	if err != nil {
		logrus.Debug("failed to subscribe to the EvtLocalReachabilityChanged")
		return
	}
		...
		case ev, ok := <-subReachability.Out():
			...
			case network.ReachabilityPrivate, network.ReachabilityUnknown:
				...
				if errors.Is(err, errAlreadyRunning) {
					logrus.Debug("tried to start already running relay finder")
				} else if err != nil {
					logrus.Error("failed to start relay finder", "error", err)
				} else {
