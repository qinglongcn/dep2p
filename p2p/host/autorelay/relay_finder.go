...
import (
	...
	basic "github.com/bpfs/dep2p/p2p/host/basic"
	"github.com/bpfs/dep2p/p2p/host/eventbus"
	circuitv2 "github.com/bpfs/dep2p/p2p/protocol/circuitv2/client"
	circuitv2_proto "github.com/bpfs/dep2p/p2p/protocol/circuitv2/proto"
...
func newRelayFinder(host *basic.BasicHost, peerSource PeerSource, conf *config) *relayFinder {
	if peerSource == nil {
		panic("Can not create a new relayFinder. Need a Peer Source fn or a list of static relays. Refer to the documentation around `dep2p.EnableAutoRelay`")
	}
...
func (rf *relayFinder) background(ctx context.Context) {
	...
	if err != nil {
		logrus.Error("failed to subscribe to the EvtPeerConnectednessChanged")
		return
	}
	...
	for {
		...
		case ev, ok := <-subConnectedness.Out():
			...
			if rf.usingRelay(evt.Peer) { // we were disconnected from a relay
				logrus.Debug("disconnected from relay", "id", evt.Peer)
...
func (rf *relayFinder) clearOldCandidates(now time.Time) time.Time {
	...
	for id, cand := range rf.candidates {
		...
		if expiry.After(now) {
			...
			logrus.Debug("deleting candidate due to age", "id", id)
			deleted = true
...
func (rf *relayFinder) clearBackoff(now time.Time) time.Time {
	...
	for id, t := range rf.backoff {
		...
		if expiry.After(now) {
			...
			logrus.Debug("removing backoff for node", "id", id)
...
func (rf *relayFinder) findNodes(ctx context.Context, peerSourceRateLimiter <-chan struct{}) {
	...
	for {
		...
		case pi, ok := <-peerChan:
			...
			logrus.Debug("found node", "id", pi.ID)
			rf.candidateMx.Lock()
			numCandidates := len(rf.candidates)
			backoffStart, isOnBackoff := rf.backoff[pi.ID]
			rf.candidateMx.Unlock()
			if isOnBackoff {
				logrus.Debug("skipping node that we recently failed to obtain a reservation with", "id", pi.ID, "last attempt", rf.conf.clock.Since(backoffStart))
				continue
			}
			if numCandidates >= rf.conf.maxCandidates {
				logrus.Debug("skipping node. Already have enough candidates", "id", pi.ID, "num", numCandidates, "max", rf.conf.maxCandidates)
				continue
			}
			rf.refCount.Add(1)
			wg.Add(1)
...
func (rf *relayFinder) handleNewNode(ctx context.Context, pi peer.AddrInfo) (added bool) {
	...
	if err != nil {
		logrus.Debugf("node %s not accepted as a candidate: %s", pi.ID, err)
	...
	logrus.Debug("node supports relay protocol", "peer", pi.ID, "supports circuit v2", supportsV2)
...
func (rf *relayFinder) maybeConnectToRelay(ctx context.Context) {
	...
	for _, cand := range candidates {
		...
		if err != nil {
			logrus.Debug("failed to connect to relay", "peer", id, "error", err)
			rf.notifyMaybeNeedNewCandidates()
			rf.metricsTracer.ReservationRequestFinished(false, err)
			continue
		}
		logrus.Debug("adding new relay", "id", id)
		rf.relayMx.Lock()
		rf.relays[id] = rsvp
		numRelays := len(rf.relays)
		rf.relayMx.Unlock()
...
func (rf *relayFinder) refreshRelayReservation(ctx context.Context, p peer.ID) error {
	...
	rf.relayMx.Lock()
	if err != nil {
		logrus.Debug("failed to refresh relay slot reservation", "relay", p, "error", err)
	...
	logrus.Debug("refreshed relay slot reservation", "relay", p)
	rf.relays[p] = rsvp
	rf.relayMx.Unlock()
	return nil
}
...
func (rf *relayFinder) Start() error {
	...
	logrus.Debug("starting relay finder")
...
func (rf *relayFinder) Stop() error {
	rf.ctxCancelMx.Lock()
	defer rf.ctxCancelMx.Unlock()
	logrus.Debug("stopping relay finder")
	if rf.ctxCancel != nil {
		rf.ctxCancel()
	}
	rf.refCount.Wait()
