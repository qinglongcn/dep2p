...
func (rh *RoutedHost) Connect(ctx context.Context, pi peer.AddrInfo) error {
	...
	for _, addr := range addrs {
		...
		if err != nil {
			logrus.Debugf("failed to parse relay ID in address %s: %s", relay, err)
			continue
		}
		...
		if err != nil {
			logrus.Debugf("failed to find relay %s: %s", relay, err)
			continue
		}
	...
	if cerr := rh.host.Connect(ctx, pi); cerr != nil {
		...
		if err != nil {
			logrus.Debugf("failed to find more peer addresses %s: %s", pi.ID, err)
			return cerr
		}
...
func (rh *RoutedHost) Close() error {
	// no need to close DeP2PRouting. we dont own it.
