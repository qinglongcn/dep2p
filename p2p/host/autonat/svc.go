...
import (
	...
	"github.com/bpfs/dep2p/p2p/host/autonat/pb"
...
func (as *autoNATService) handleStream(s network.Stream) {
	if err := s.Scope().SetService(ServiceName); err != nil {
		logrus.Debugf("error attaching stream to autonat service: %s", err)
		s.Reset()
		return
	}

	if err := s.Scope().ReserveMemory(maxMsgSize, network.ReservationPriorityAlways); err != nil {
		logrus.Debugf("error reserving memory for autonat stream: %s", err)
		s.Reset()
		return
	}
	...
	logrus.Debugf("New stream from %s", pid)
	...
	err := r.ReadMsg(&req)
	if err != nil {
		logrus.Debugf("Error reading message from %s: %s", pid, err.Error())
		s.Reset()
		return
	}
	...
	if t != pb.Message_DIAL {
		logrus.Debugf("Unexpected message from %s: %s (%d)", pid, t.String(), t)
		s.Reset()
		return
	}
	...
	err = w.WriteMsg(&res)
	if err != nil {
		logrus.Debugf("Error writing response to %s: %s", pid, err.Error())
		s.Reset()
		return
	}
...
func (as *autoNATService) handleDial(p peer.ID, obsaddr ma.Multiaddr, mpi *pb.Message_PeerInfo) *pb.Message_DialResponse {
	...
	for _, maddr := range mpi.GetAddrs() {
		...
		if err != nil {
			logrus.Debugf("Error parsing multiaddr: %s", err.Error())
			continue
		}
...
func (as *autoNATService) doDial(pi peer.AddrInfo) *pb.Message_DialResponse {
	...
	if err != nil {
		logrus.Debugf("error dialing %s: %s", pi.ID, err.Error())
