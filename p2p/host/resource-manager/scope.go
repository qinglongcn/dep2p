...
func (rc *resources) releaseMemory(size int64) {
	...
	if rc.memory < 0 {
		logrus.Warn("BUG: too much memory released")
		rc.memory = 0
	}
}
...
func (rc *resources) removeStreams(incount, outcount int) {
	...
	if rc.nstreamsIn < 0 {
		logrus.Warn("BUG: too many inbound streams released")
		rc.nstreamsIn = 0
	}
	if rc.nstreamsOut < 0 {
		logrus.Warn("BUG: too many outbound streams released")
		rc.nstreamsOut = 0
	}
}
...
func (rc *resources) removeConns(incount, outcount, fdcount int) {
	...
	if rc.nconnsIn < 0 {
		logrus.Warn("BUG: too many inbound connections released")
		rc.nconnsIn = 0
	}
	if rc.nconnsOut < 0 {
		logrus.Warn("BUG: too many outbound connections released")
		rc.nconnsOut = 0
	}
	if rc.nfd < 0 {
		logrus.Warn("BUG: too many file descriptors released")
		rc.nfd = 0
	}
}
...
func (s *resourceScope) ReserveMemory(size int, prio uint8) error {
	...
	if err := s.rc.reserveMemory(int64(size), prio); err != nil {
		logrus.Debugf("blocked memory reservation %s", logValuesMemoryLimit(s.name, "", s.rc.stat(), err)...)
...
func (s *resourceScope) reserveMemoryForEdges(size int, prio uint8) error {
	...
	for _, e := range s.edges {
		...
		if err != nil {
			logrus.Debugf("blocked memory reservation from constraining edge %s", logValuesMemoryLimit(s.name, e.name, stat, err)...)
			break
		}
...
func (s *resourceScope) AddStream(dir network.Direction) error {
	...
	if err := s.rc.addStream(dir); err != nil {
		logrus.Debugf("blocked stream %s", logValuesStreamLimit(s.name, "", dir, s.rc.stat(), err)...)
...
func (s *resourceScope) addStreamForEdges(dir network.Direction) error {
	...
	for _, e := range s.edges {
		...
		if err != nil {
			logrus.Debugf("blocked stream from constraining edge %s", logValuesStreamLimit(s.name, e.name, dir, stat, err)...)
			break
		}
		reserved++
	}
...
func (s *resourceScope) AddConn(dir network.Direction, usefd bool) error {
	...
	if err := s.rc.addConn(dir, usefd); err != nil {
		logrus.Debugf("blocked connection %s", logValuesConnLimit(s.name, "", dir, usefd, s.rc.stat(), err)...)
...
func (s *resourceScope) addConnForEdges(dir network.Direction, usefd bool) error {
	...
	for _, e := range s.edges {
		...
		if err != nil {
			logrus.Debugf("blocked connection from constraining edge %s", logValuesConnLimit(s.name, e.name, dir, usefd, stat, err)...)
			break
		}
		reserved++
	}
