...
func NewBlankHost(n network.Network, options ...Option) *BlankHost {
	...
	if err := bh.initSignedRecord(); err != nil {
		logrus.Errorf("error creating blank host, err=%s", err)
		return nil
	}
...
func (bh *BlankHost) initSignedRecord() error {
	...
	if !ok {
		logrus.Error("peerstore does not support signed records")
	...
	if err != nil {
		logrus.Errorf("failed to create signed record for self, err=%s", err)
	...
	if err != nil {
		logrus.Errorf("failed to persist signed record to peerstore,err=%s", err)
...
func (bh *BlankHost) Addrs() []ma.Multiaddr {
	...
	if err != nil {
		logrus.Debug("error retrieving network interface addrs: ", err)
		return nil
	}
...
func (bh *BlankHost) newStreamHandler(s network.Stream) {
	...
	if err != nil {
		logrus.Info("protocol negotiation failed", "error", err)
		s.Reset()
		return
	}
