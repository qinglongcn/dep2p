...
import (
	...
	pstore "github.com/bpfs/dep2p/core/peerstore"
...
//     permanent, popular values used in other dep2p modules. In this cited case, optimizing with lookahead windows
//     makes little sense.
...
// See https://godoc.org/github.com/bpfs/dep2p/core/peerstore#CertifiedAddrBook for more details.
...
func (ab *dsAddrBook) latestPeerRecordSeq(p peer.ID) uint64 {
	...
	if err != nil {
		...
		// case.
		logrus.Error("unable to load record", "peer", p, "error", err)
		return 0
	}
	pr.RLock()
	defer pr.RUnlock()
...
func (ab *dsAddrBook) GetPeerRecord(p peer.ID) *record.Envelope {
	pr, err := ab.loadRecord(p, true, false)
	if err != nil {
		logrus.Errorf("unable to load record for peer %s: %v", p, err)
		return nil
	}
	pr.RLock()
	defer pr.RUnlock()
	...
	if err != nil {
		logrus.Errorf("error unmarshaling stored signed peer record for peer %s: %v", p, err)
		return nil
	}
	return state
}
...
func (ab *dsAddrBook) UpdateAddrs(p peer.ID, oldTTL time.Duration, newTTL time.Duration) {
	pr, err := ab.loadRecord(p, true, false)
	if err != nil {
		logrus.Errorf("failed to update ttls for peer %s: %s\n", p, err)
		return
	}
...
func (ab *dsAddrBook) Addrs(p peer.ID) []ma.Multiaddr {
	...
	if err != nil {
		logrus.Warnf("failed to load peerstore entry for peer %s while querying addrs, err: %v", p, err)
		return nil
	}
	...
	for i, a := range pr.Addrs {
		...
		if err != nil {
			logrus.Warn("failed to parse peerstore entry for peer %v while querying addrs, err: %v", p, err)
			return nil
		}
	}
	return addrs
...
func (ab *dsAddrBook) PeersWithAddrs() peer.IDSlice {
	...
	if err != nil {
		logrus.Errorf("error while retrieving peers with addresses: %v", err)
	}
	return ids
}
...
func (ab *dsAddrBook) ClearAddrs(p peer.ID) {
	...
	if err := ab.ds.Delete(context.TODO(), key); err != nil {
		logrus.Errorf("failed to clear addresses for peer %s: %v", p, err)
	}
}
...
func cleanAddrs(addrs []ma.Multiaddr, pid peer.ID) []ma.Multiaddr {
	...
	for _, addr := range addrs {
		...
		if addr == nil {
			logrus.Warn("Was passed a nil multiaddr", "peer", pid)
			continue
		}
		if addrPid != "" && addrPid != pid {
			logrus.Warnf("Was passed p2p address with a different peerId. found: %s, expected: %s", addrPid, pid)
			continue
		}
