...
import (
	...
	"github.com/bpfs/dep2p/p2p/host/peerstore/pstoreds/pb"
...
func (gc *dsAddrBookGc) purgeLookahead() {
	...
	if err != nil {
		logrus.Warnf("failed while creating batch to purge GC entries: %v", err)
	}
	...
	dropInError := func(key ds.Key, err error, msg string) {
		if err != nil {
			logrus.Warnf("failed while %s record with GC key: %v, err: %v; deleting", msg, key, err)
		}
		if err = batch.Delete(context.TODO(), key); err != nil {
			logrus.Warnf("failed to delete corrupt GC lookahead entry: %v, err: %v", key, err)
		}
	}
	...
	dropOrReschedule := func(key ds.Key, ar *addrsRecord) {
		if err := batch.Delete(context.TODO(), key); err != nil {
			logrus.Warnf("failed to delete lookahead entry: %v, err: %v", key, err)
		}
		...
		if len(ar.Addrs) != 0 && ar.Addrs[0].Expiry <= gc.currWindowEnd {
			...
			if err := batch.Put(context.TODO(), gcKey, []byte{}); err != nil {
				logrus.Warnf("failed to add new GC key: %v, err: %v", gcKey, err)
			}
		}
	}
	...
	if err != nil {
		logrus.Warnf("failed while fetching entries to purge: %v", err)
		return
	}
	defer results.Close()
	...
	for result := range results.Next() {
		...
		if err != nil {
			...
			logrus.Warnf("failed while parsing timestamp from key: %v, err: %v", result.Key, err)
			continue
		} else if ts > now {
		...
		if err != nil {
			...
			logrus.Warnf("failed while parsing b32 peer ID from key: %v, err: %v", result.Key, err)
			continue
		}
		...
		if err != nil {
			...
			logrus.Warnf("failed while decoding peer ID from key: %v, err: %v", result.Key, err)
			continue
		}
		...
		if cached, ok := gc.ab.cache.Peek(id); ok {
			...
			if cached.clean(gc.ab.clock.Now()) {
				if err = cached.flush(batch); err != nil {
					logrus.Warnf("failed to flush entry modified by GC for peer: %s, err: %v", id, err)
				}
			}
	...
	if err = batch.Commit(context.TODO()); err != nil {
		logrus.Warnf("failed to commit GC purge batch: %v", err)
	}
}
...
func (gc *dsAddrBookGc) purgeStore() {
	...
	if err != nil {
		logrus.Warnf("failed while opening iterator: %v", err)
		return
	}
	defer results.Close()
	...
	for result := range results.Next() {
		...
		if err := record.flush(batch); err != nil {
			logrus.Warnf("failed to flush entry modified by GC for peer: &v, err: %v", id, err)
		}
...
func (gc *dsAddrBookGc) populateLookahead() {
	...
	if err != nil {
		logrus.Warnf("failed while querying to populate lookahead GC window: %v", err)
		return
	}
	defer results.Close()
	...
	batch, err := newCyclicBatch(gc.ab.ds, defaultOpsPerCyclicBatch)
	if err != nil {
		logrus.Warnf("failed while creating batch to populate lookahead GC window: %v", err)
		return
	}
	...
	for result := range results.Next() {
		...
		if cached, ok := gc.ab.cache.Peek(id); ok {
			...
			if err = batch.Put(context.TODO(), gcKey, []byte{}); err != nil {
				logrus.Warnf("failed while inserting GC entry for peer: %s, err: %v", id, err)
			}
			cached.RUnlock()
			continue
		}
		...
		if err != nil {
			logrus.Warnf("failed which getting record from store for peer: %s, err: %v", id, err)
			continue
		}
		if err := proto.Unmarshal(val, record); err != nil {
			logrus.Warnf("failed while unmarshalling record from store for peer: %s, err: %v", id, err)
			continue
		}
	...
	if err = batch.Commit(context.TODO()); err != nil {
		logrus.Warnf("failed to commit GC lookahead batch: %v", err)
	}
