...
func (kb *dsKeyBook) PubKey(p peer.ID) ic.PubKey {
	...
	if value, err := kb.ds.Get(context.TODO(), key); err == nil {
		...
		if err != nil {
			logrus.Errorf("error when unmarshalling pubkey from datastore for peer %s: %s\n", p, err)
		}
		...
		case peer.ErrNoPublicKey:
			return nil
		default:
			logrus.Errorf("error when extracting pubkey from peer ID for peer %s: %s\n", p, err)
			return nil
		}
		pkb, err := ic.MarshalPublicKey(pk)
		if err != nil {
			logrus.Errorf("error when turning extracted pubkey into bytes for peer %s: %s\n", p, err)
			return nil
		}
		if err := kb.ds.Put(context.TODO(), key, pkb); err != nil {
			logrus.Errorf("error when adding extracted pubkey to peerstore for peer %s: %s\n", p, err)
			return nil
		}
	} else {
		logrus.Errorf("error when fetching pubkey from datastore for peer %s: %s\n", p, err)
	}
...
func (kb *dsKeyBook) AddPubKey(p peer.ID, pk ic.PubKey) error {
	...
	if err != nil {
		logrus.Errorf("error while converting pubkey byte string for peer %s: %s\n", p, err)
		return err
	}
	if err := kb.ds.Put(context.TODO(), peerToKey(p, pubSuffix), val); err != nil {
		logrus.Errorf("error while updating pubkey in datastore for peer %s: %s\n", p, err)
		return err
	}
	return nil
}
...
func (kb *dsKeyBook) AddPrivKey(p peer.ID, sk ic.PrivKey) error {
	...
	if err != nil {
		logrus.Errorf("error while converting privkey byte string for peer %s: %s\n", p, err)
		return err
	}
	if err := kb.ds.Put(context.TODO(), peerToKey(p, privSuffix), val); err != nil {
		logrus.Errorf("error while updating privkey in datastore for peer %s: %s\n", p, err)
	}
	return err
}
...
func (kb *dsKeyBook) PeersWithKeys() peer.IDSlice {
	...
	if err != nil {
		logrus.Errorf("error while retrieving peers with keys: %v", err)
	}
	return ids
}
