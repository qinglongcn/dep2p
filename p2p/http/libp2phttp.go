...
import (
	...
	host "github.com/bpfs/dep2p/core/host"
	...
	gostream "github.com/bpfs/dep2p/p2p/net/gostream"
...
// streamHostListen retuns a net.Listener that listens on dep2p streams for HTTP/1.1 messages.
...
// Host is a dep2p host for request/responses with HTTP semantics. This is
...
func (h *Host) setupListeners(listenerErrCh chan error) error {
	for _, addr := range h.ListenAddrs {
		...
		if parsedAddr.useHTTPS {
			...
			logrus.Warnf("Not serving insecure HTTP on %s. Prefer an HTTPS endpoint.", listenAddr)
		}
	}
	return nil
}
...
func (h *Host) Serve() error {
	...
	h.serveMuxInit()
	h.ServeMux.Handle("/.well-known/dep2p", &h.WellKnownHandler)
...
// manages the .well-known/dep2p mapping.
...
// given path. Automatically manages the .well-known/dep2p mapping.
// http.StripPrefix is called on the handler, so the handler will be unaware of
// its prefix path.
...
func (h *Host) getAndStorePeerMetadata(roundtripper http.RoundTripper, server peer.ID) (PeerMeta, error) {
	...
	req, err := http.NewRequest("GET", "/.well-known/dep2p", nil)
	if err != nil {
		return nil, err
	}
