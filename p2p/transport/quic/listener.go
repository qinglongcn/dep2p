...
func (l *listener) setupConn(qconn quic.Connection) (*conn, error) {
	...
	if err != nil {
		logrus.Debug("resource manager blocked incoming connection", "addr", qconn.RemoteAddr(), "error", err)
		return nil, err
	}
...
func (l *listener) setupConnWithScope(qconn quic.Connection, connScope network.ConnManagementScope, remoteMultiaddr ma.Multiaddr) (*conn, error) {
	...
	if err := connScope.SetPeer(remotePeerID); err != nil {
		logrus.Debug("resource manager blocked incoming connection for peer", "peer", remotePeerID, "addr", qconn.RemoteAddr(), "error", err)
		return nil, err
	}
