	...
	for {
		...
		if err != nil {
			if strings.Contains(err.Error(), "use of closed network connection") {
				logrus.Debugf("readLoop exiting: socket %s closed", mux.socket.LocalAddr())
			} else {
				logrus.Errorf("error reading from socket %s: %v", mux.socket.LocalAddr(), err)
			}
			pool.Put(buf)
			return
		}
...
func (mux *UDPMux) processPacket(buf []byte, addr net.Addr) (processed bool) {
	...
	if !ok {
		logrus.Errorf("received a non-UDP address: %s", addr)
		return false
	}
	...
	if ok {
		if err := conn.Push(buf, addr); err != nil {
			logrus.Debugf("could not push packet: %v", err)
			return false
		}
		return true
	}
	...
	if !stun.IsMessage(buf) {
		logrus.Debug("incoming message is not a STUN message")
		return false
	}

	msg := &stun.Message{Raw: buf}
	if err := msg.Decode(); err != nil {
		logrus.Debugf("failed to decode STUN message: %s", err)
		return false
	}
	if msg.Type != stun.BindingRequest {
		logrus.Debugf("incoming message should be a STUN binding request, got %s", msg.Type)
		return false
	}

	ufrag, err := ufragFromSTUNMessage(msg)
	if err != nil {
		logrus.Debugf("could not find STUN username: %s", err)
		return false
	}
	...
	if connCreated {
		...
		default:
			logrus.Debug("queue full, dropping incoming candidate", "ufrag", ufrag, "addr", udpAddr)
			conn.Close()
			return false
		}
	}
