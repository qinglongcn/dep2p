...
import (
	...
	"github.com/bpfs/dep2p/p2p/transport/webrtc/udpmux"
...
func (l *listener) listen() {
	...
	for {
		...
		if err != nil {
			if l.ctx.Err() == nil {
				logrus.Debugf("accepting candidate failed: %s", err)
			}
			return
		}
		...
		go func() {
			...
			if err != nil {
				...
				logrus.Debugf("could not accept connection: %s: %v", candidate.Ufrag, err)
				return
			}

			select {
			case <-ctx.Done():
				logrus.Warn("could not push connection: ctx done")
				conn.Close()
...
func addOnConnectionStateChangeCallback(pc *webrtc.PeerConnection) <-chan error {
	...
	pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		case webrtc.PeerConnectionStateDisconnected:
			...
			logrus.Warn("peerconnection disconnected")
		}
	})
	return errC
}
