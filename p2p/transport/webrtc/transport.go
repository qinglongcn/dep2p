// Package dep2pwebrtc implements the WebRTC transport for dep2p,
// as described in https://github.com/dep2p/specs/tree/master/webrtc.
//
...
func New(privKey ic.PrivKey, psk pnet.PSK, gater connmgr.ConnectionGater, rcmgr network.ResourceManager, opts ...Option) (*WebRTCTransport, error) {
	if psk != nil {
		logrus.Error("WebRTC doesn't support private networks yet.")
...
// See https://github.com/bpfs/dep2p/issues/2446 for details.
...
func (t *WebRTCTransport) generateNoisePrologue(pc *webrtc.PeerConnection, hash crypto.Hash, inbound bool) ([]byte, error) {
	...
	if err != nil {
		logrus.Debugf("could not encode multihash for local fingerprint")
		return nil, err
	}
	remoteEncoded, err := multihash.Encode(remoteFpBytes, multihash.SHA2_256)
	if err != nil {
		logrus.Debugf("could not encode multihash for remote fingerprint")
		return nil, err
	}
