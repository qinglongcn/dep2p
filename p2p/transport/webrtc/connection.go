...
import (
	...
	"github.com/bpfs/dep2p/p2p/transport/webrtc/pb"
...
func newConnection(
	...
	pc.OnDataChannel(func(dc *webrtc.DataChannel) {
		...
		// See https://github.com/dep2p/specs/issues/575 for details.
		...
		dc.OnOpen(func() {
			rwc, err := dc.Detach()
			if err != nil {
				logrus.Warnf("could not detach datachannel: id: %d", *dc.ID())
				return
			}
			select {
			case c.acceptQueue <- dataChannel{rwc, dc}:
			default:
				logrus.Warnf("connection busy, rejecting stream")
