...
func (c *aggregatingCollector) gatherMetrics(now time.Time) {
	...
	for _, conn := range c.conns {
		...
		if err != nil {
			...
			logrus.Errorf("Failed to get TCP info: %s", err)
			continue
		}
		if hasSegmentCounter {
...
func (c *aggregatingCollector) Collect(metrics chan<- prometheus.Metric) {
	...
	if hasSegmentCounter {
		...
		if err != nil {
			logrus.Errorf("creating tcp_sent_segments_total metric failed: %v", err)
			return
		}
		segsRcvdMetric, err := prometheus.NewConstMetric(segsRcvdDesc, prometheus.CounterValue, float64(c.segsRcvd))
		if err != nil {
			logrus.Errorf("creating tcp_rcvd_segments_total metric failed: %v", err)
			return
		}
	...
	if hasByteCounter {
		...
		if err != nil {
			logrus.Errorf("creating tcp_sent_bytes metric failed: %v", err)
			return
		}
		bytesRcvdMetric, err := prometheus.NewConstMetric(bytesRcvdDesc, prometheus.CounterValue, float64(c.bytesRcvd))
		if err != nil {
			logrus.Errorf("creating tcp_rcvd_bytes metric failed: %v", err)
			return
		}
