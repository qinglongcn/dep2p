...
import (
	...
	"github.com/bpfs/dep2p/p2p/security/noise/pb"
...
func newListener(reuseListener quicreuse.Listener, t *transport, isStaticTLSConf bool) (tpt.Listener, error) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				logrus.Debug("serving failed", "addr", ln.Addr(), "error", err)
				return
			}
...
func (l *listener) httpHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logrus.Error("converting remote address failed", "remote", r.RemoteAddr, "error", err)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		logrus.Debug("resource manager blocked incoming connection", "addr", r.RemoteAddr, "error", err)
...
func (l *listener) httpHandlerWithConnScope(w http.ResponseWriter, r *http.Request, connScope network.ConnManagementScope) error {
	...
	if err != nil {
		logrus.Debug("upgrade failed", "error", err)
	...
	if err != nil {
		cancel()
		logrus.Debug("handshake failed", "error", err)
	...
	if err := connScope.SetPeer(sconn.RemotePeer()); err != nil {
		logrus.Debug("resource manager blocked incoming connection for peer", "peer", sconn.RemotePeer(), "addr", r.RemoteAddr, "error", err)
		sess.CloseWithError(1, "")
		return err
	}
	...
	case l.queue <- conn:
	default:
		logrus.Debug("accept queue full, dropping incoming connection", "peer", sconn.RemotePeer(), "addr", r.RemoteAddr, "error", err)
		sess.CloseWithError(1, "")
