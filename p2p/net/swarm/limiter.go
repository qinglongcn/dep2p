...
func newDialLimiter(df dialfunc) *dialLimiter {
	...
	if env := os.Getenv("DeP2P_SWARM_FD_LIMIT"); env != "" {
...
func (dl *dialLimiter) freeFDToken() {
	logrus.Debugf("[limiter] freeing FD token; waiting: %d; consuming: %d", len(dl.waitingOnFd), dl.fdConsuming)
	dl.fdConsuming--
...
func (dl *dialLimiter) freePeerToken(dj *dialJob) {
	logrus.Debugf("[limiter] freeing peer token; peer %s; addr: %s; active for peer: %d; waiting on peer limit: %d",
...
func (dl *dialLimiter) addCheckFdLimit(dj *dialJob) {
	if dl.shouldConsumeFd(dj.addr) {
		if dl.fdConsuming >= dl.fdLimit {
			logrus.Debugf("[limiter] blocked dial waiting on FD token; peer: %s; addr: %s; consuming: %d; "+
				"limit: %d; waiting: %d", dj.peer, dj.addr, dl.fdConsuming, dl.fdLimit, len(dl.waitingOnFd))
			dl.waitingOnFd = append(dl.waitingOnFd, dj)
			return
		}

		logrus.Debugf("[limiter] taking FD token: peer: %s; addr: %s; prev consuming: %d",
			dj.peer, dj.addr, dl.fdConsuming)
		// take token
		dl.fdConsuming++
	}

	logrus.Debugf("[limiter] executing dial; peer: %s; addr: %s; FD consuming: %d; waiting: %d",
...
func (dl *dialLimiter) addCheckPeerLimit(dj *dialJob) {
	if dl.activePerPeer[dj.peer] >= dl.perPeerLimit {
		logrus.Debugf("[limiter] blocked dial waiting on peer limit; peer: %s; addr: %s; active: %d; "+
...
func (dl *dialLimiter) AddDialJob(dj *dialJob) {
	...
	logrus.Debugf("[limiter] adding a dial job through limiter: %v", dj.addr)
...
func (dl *dialLimiter) clearAllPeerDials(p peer.ID) {
	...
	logrus.Debugf("[limiter] clearing all peer dials: %v", p)
