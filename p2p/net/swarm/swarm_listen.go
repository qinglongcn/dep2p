...
func (s *Swarm) Listen(addrs ...ma.Multiaddr) error {
	...
	for i, e := range errs {
		if e != nil {
			logrus.Warn("listening failed", "on", addrs[i], "error", errs[i])
		}
	}
...
func (s *Swarm) AddListenAddr(a ma.Multiaddr) error {
	...
	go func() {
		defer func() {
			...
			if ok {
				list.Close()
				logrus.Errorf("swarm listener unintentionally closed")
			}
		...
		for {
			...
			if err != nil {
				if !errors.Is(err, transport.ErrListenerClosed) {
					logrus.Errorf("swarm listener for %s accept error: %s", a, err)
				}
				return
			}
			...
			logrus.Debugf("swarm listener accepted connection: %s <-> %s", c.LocalMultiaddr(), c.RemoteMultiaddr())
			s.refs.Add(1)
			go func() {
				defer s.refs.Done()
				...
				case ErrSwarmClosed:
					...
					return
				default:
					logrus.Warn("adding connection failed", "to", a, "error", err)
					return
				}
			}()
		}
