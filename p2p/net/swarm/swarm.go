	...
	for l := range listeners {
		go func(l transport.Listener) {
			if err := l.Close(); err != nil && err != transport.ErrListenerClosed {
				logrus.Errorf("error when shutting down listener: %s", err)
			}
		}(l)
	}
		...
		for _, c := range cs {
			go func(c *Conn) {
				if err := c.Close(); err != nil {
					logrus.Errorf("error when shutting down connection: %s", err)
				}
			}(c)
		}
	}
		...
		if closer, ok := t.(io.Closer); ok {
			...
			go func(c io.Closer) {
				...
				if err := closer.Close(); err != nil {
					logrus.Errorf("error when closing down transport %T: %s", c, err)
				}
			}(closer)
		}
	}
...
func (s *Swarm) addConn(tc transport.CapableConn, dir network.Direction) (*Conn, error) {
	...
	if s.gater != nil {
		if allow, _ := s.gater.InterceptUpgraded(c); !allow {
			...
			err := tc.Close()
			if err != nil {
				logrus.Warnf("failed to close connection with peer %s and addr %s; err: %s", p, addr, err)
			}
...
func (s *Swarm) NewStream(ctx context.Context, p peer.ID) (network.Stream, error) {
	logrus.Debugf("[%s] opening stream to peer [%s]", s.local, p)
