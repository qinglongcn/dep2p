	...
	for {
		...
		case <-dialTimer.Ch():
			...
			for _, adelay := range dq.NextBatch() {
				...
				ad, ok := w.trackedDials[string(adelay.Addr.Bytes())]
				if !ok {
					logrus.Errorf("SWARM BUG: no entry for address %s in trackedDials", adelay.Addr)
					continue
				}
				ad.dialed = true
		...
		case res := <-w.resch:
			...
			if !ok {
				logrus.Errorf("SWARM BUG: no entry for address %s in trackedDials", res.Addr)
				if res.Conn != nil {
					res.Conn.Close()
				}
				dialsInFlight--
			...
			if res.Err != ErrDialRefusedBlackHole && res.Err != context.Canceled && !w.connected {
				...
				logrus.Errorf("SWARM BUG: unexpected ErrDialRefusedBlackHole while dialing peer %s to addr %s",
					w.peer, res.Addr)
			}
