...
import (
	...
	"github.com/bpfs/dep2p/core/canonicallog"
...
func (s *Swarm) dialPeer(ctx context.Context, p peer.ID) (*Conn, error) {
	logrus.Debug("dialing peer", "from", s.local, "to", p)
	err := p.Validate()
	if err != nil {
		return nil, err
	}
	...
	if s.gater != nil && !s.gater.InterceptPeerDial(p) {
		logrus.Debugf("gater disallowed outbound connection to peer %s", p)
	...
	if err == nil {
		...
		if conn.RemotePeer() != p {
			conn.Close()
			logrus.Error("Handshake failed to properly authenticate peer", "authenticated", conn.RemotePeer(), "expected", p)
			return nil, fmt.Errorf("unexpected peer")
		}
		return conn, nil
	}

	logrus.Debugf("network for %s finished dialing %s", s.local, p)
...
func (s *Swarm) resolveAddrs(ctx context.Context, pi peer.AddrInfo) ([]ma.Multiaddr, error) {
	...
	for len(toResolve) > 0 {
		...
		if resolveSteps >= maxAddressResolution {
			logrus.Warnf(
				"peer %s asked us to resolve too many addresses: %d/%d",
				pi.ID,
				resolveSteps,
		...
		if ok {
			...
			if err != nil {
				logrus.Warnf("Failed to resolve multiaddr %s by transport %v: %v", addr, tpt, err)
				continue
			}
			var added bool
		...
		if err != nil {
			logrus.Infof("error resolving %s: %s", reqaddr, err)
		}
		...
		for _, res := range resaddrs {
			...
			if err != nil {
				logrus.Infof("error parsing %s: %s", res, err)
			}
...
func (s *Swarm) dialAddr(ctx context.Context, p peer.ID, addr ma.Multiaddr, updCh chan<- transport.DialUpdate) (transport.CapableConn, error) {
	...
	if err := ctx.Err(); err != nil {
		logrus.Debugf("%s swarm not dialing. Context cancelled: %v. %s %s", s.local, err, p, addr)
		return nil, err
	}
	logrus.Debugf("%s swarm dialing %s %s", s.local, p, addr)
...
// TODO We should have a `IsFdConsuming() bool` method on the `Transport` interface in dep2p/core/transport.
