...
import (
	...
	"github.com/bpfs/dep2p/util/nat"
)
...
func DiscoverNAT(ctx context.Context) (*NAT, error) {
	...
	if err != nil {
		logrus.Debug("DiscoverGateway address error:", err)
	} else {
		logrus.Debug("DiscoverGateway address:", addr)
	}
...
func (nat *NAT) establishMapping(ctx context.Context, protocol string, internalPort int) (externalPort int) {
	logrus.Debugf("Attempting port map: %s/%d", protocol, internalPort)
	const comment = "dep2p"
	...
	if err != nil || externalPort == 0 {
		// TODO: log.Event
		if err != nil {
			logrus.Warnf("failed to establish port mapping: %s", err)
		} else {
			logrus.Warnf("failed to establish port mapping: newport = 0")
		}
	...
	logrus.Debugf("NAT Mapping: %d --> %d (%s)", externalPort, internalPort, protocol)
	return externalPort
}
