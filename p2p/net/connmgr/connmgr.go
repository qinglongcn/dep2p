...
func (cm *BasicConnMgr) memoryEmergency() {
	...
	if target < 0 {
		logrus.Warn("Low on memory, but we only have a few connections", "num", connCount, "low watermark", cm.cfg.lowWater)
		return
	} else {
		logrus.Warnf("Low on memory. Closing %d connections.", target)
	}
	...
	for _, c := range cm.getConnsToCloseEmergency(target) {
		logrus.Info("low on memory. closing conn", "peer", c.RemotePeer())
		c.Close()
	}
...
func (cm *BasicConnMgr) trim() {
	...
	for _, c := range cm.getConnsToClose() {
		logrus.Debug("closing conn", "peer", c.RemotePeer())
		c.Close()
	}
}
...
func (cm *BasicConnMgr) getConnsToClose() []network.Conn {
	...
	if int(cm.connCount.Load()) <= cm.cfg.lowWater {
		logrus.Info("open connection count below limit")
		return nil
	}
	...
	if ncandidates < cm.cfg.lowWater {
		logrus.Info("open connection count above limit but too many are in the grace period")
...
func (cm *BasicConnMgr) UntagPeer(p peer.ID, tag string) {
	...
	pi, ok := s.peers[p]
	if !ok {
		logrus.Info("tried to remove tag from untracked peer: ", p)
		return
	}
...
func (nn *cmNotifee) Connected(n network.Network, c network.Conn) {
	...
	_, ok = pinfo.conns[c]
	if ok {
		logrus.Error("received connected notification for conn we are already tracking: ", p)
		return
	}
...
func (nn *cmNotifee) Disconnected(n network.Network, c network.Conn) {
	...
	cinf, ok := s.peers[p]
	if !ok {
		logrus.Error("received disconnected notification for peer we are not tracking: ", p)
		return
	}

	_, ok = cinf.conns[c]
	if !ok {
		logrus.Error("received disconnected notification for conn we are not tracking: ", p)
		return
	}
