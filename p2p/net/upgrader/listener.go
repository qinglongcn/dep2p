	...
	for l.ctx.Err() == nil {
		...
		if err != nil {
			...
			if catcher.IsTemporary(err) {
				logrus.Infof("temporary accept error: %s", err)
				continue
			}
			l.err = err
			return
		...
		if l.upgrader.connGater != nil && !l.upgrader.connGater.InterceptAccept(maconn) {
			logrus.Debugf("gater blocked incoming connection on local addr %s from %s",
				maconn.LocalMultiaddr(), maconn.RemoteMultiaddr())
			if err := maconn.Close(); err != nil {
				logrus.Warnf("failed to close incoming connection rejected by gater: %s", err)
			}
			continue
		}
		...
		if err != nil {
			logrus.Debug("resource manager blocked accept of new connection", "error", err)
			if err := maconn.Close(); err != nil {
				logrus.Warnf("failed to incoming connection rejected by resource manager: %s", err)
			}
			continue
		}
		...
		logrus.Debugf("listener %s got connection: %s <---> %s",
			l,
		...
		go func() {
			...
			if err != nil {
				...
				logrus.Debugf("accept upgrade error: %s (%s <--> %s)",
					err,
					maconn.LocalMultiaddr(),
					maconn.RemoteMultiaddr())
				connScope.Done()
			...
			logrus.Debugf("listener %s accepted connection: %s", l, conn)
			...
			case <-ctx.Done():
				if l.ctx.Err() == nil {
					...
					logrus.Warn("listener dropped connection due to slow accept")
				}
