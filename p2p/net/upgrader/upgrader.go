...
// UpgradeListener upgrades the passed multiaddr-net listener into a full dep2p-transport listener.
...
// Upgrade upgrades the multiaddr/net connection into a full dep2p-transport connection.
...
func (u *upgrader) upgrade(ctx context.Context, t transport.Transport, maconn manet.Conn, dir network.Direction, p peer.ID, connScope network.ConnManagementScope) (transport.CapableConn, error) {
	...
	if u.psk != nil {
		...
		logrus.Error("tried to dial with no Private Network Protector but usage of Private Networks is forced by the environment")
	...
	if u.connGater != nil && !u.connGater.InterceptSecured(dir, sconn.RemotePeer(), maconn) {
		if err := maconn.Close(); err != nil {
			logrus.Error("failed to close connection", "peer", p, "addr", maconn.RemoteMultiaddr(), "error", err)
		}
	...
	if connScope.PeerScope() == nil {
		if err := connScope.SetPeer(sconn.RemotePeer()); err != nil {
			logrus.Debug("resource manager blocked connection for peer", "peer", sconn.RemotePeer(), "addr", conn.RemoteAddr(), "error", err)
			if err := maconn.Close(); err != nil {
				logrus.Error("failed to close connection", "peer", p, "addr", maconn.RemoteMultiaddr(), "error", err)
			}
