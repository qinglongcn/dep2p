...
func (pn *peernet) connect(p peer.ID) (*conn, error) {
	...
	logrus.Debugf("%s (newly) dialing %s", pn.peer, p)
	...
	logrus.Debugf("%s dialing %s openingConn", pn.peer, p)
...
func (pn *peernet) openConn(r peer.ID, l *link) (*conn, error) {
	...
	logrus.Debugf("%s opening connection to %s", pn.LocalPeer(), lc.RemotePeer())
	abort := func() {
		_ = lc.Close()
		_ = rc.Close()
	}
...
func (pn *peernet) remoteOpenedConn(c *conn) {
	logrus.Debugf("%s accepting connection from %s", pn.LocalPeer(), c.RemotePeer())
	pn.addConn(c)
}
