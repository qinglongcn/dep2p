...
import (
	...
	"github.com/bpfs/dep2p/util/zeroconf"
...
func (s *mdnsService) startServer() error {
	...
	server, err := zeroconf.RegisterProxy(
		...
		s.serviceName,
		mdnsDomain,
		4001, // we have to pass in a port number here, but dep2p only uses the TXT records
		s.peerName,
		ips,
		txts,
		nil,
...
func (s *mdnsService) startResolver(ctx context.Context) {
		...
		for entry := range entryChan {
			...
			for _, s := range entry.Text {
				if !strings.HasPrefix(s, dnsaddrPrefix) {
					logrus.Debug("missing dnsaddr prefix")
					continue
				}
				addr, err := ma.NewMultiaddr(s[len(dnsaddrPrefix):])
				if err != nil {
					logrus.Debugf("failed to parse multiaddr: %s", err)
					continue
				}
			...
			if err != nil {
				logrus.Debugf("failed to get peer info: %s", err)
				continue
			}
	...
	go func() {
		...
		if err := zeroconf.Browse(ctx, s.serviceName, mdnsDomain, entryChan); err != nil {
			logrus.Debugf("zeroconf browsing failed: %s", err)
		}
	}()
}
