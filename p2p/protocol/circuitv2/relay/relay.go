...
import (
	...
	pbv2 "github.com/bpfs/dep2p/p2p/protocol/circuitv2/pb"
	"github.com/bpfs/dep2p/p2p/protocol/circuitv2/proto"
	"github.com/bpfs/dep2p/p2p/protocol/circuitv2/util"
	"github.com/sirupsen/logrus"

	ma "github.com/multiformats/go-multiaddr"
	manet "github.com/multiformats/go-multiaddr/net"
	asnutil "github.com/bpfs/dep2p/util/asn"
	"github.com/bpfs/dep2p/util/pool"
)
...
func (r *Relay) handleStream(s network.Stream) {
	logrus.Infof("new relay stream from: %s", s.Conn().RemotePeer())

	if err := s.Scope().SetService(ServiceName); err != nil {
		logrus.Debugf("error attaching stream to relay service: %s", err)
		s.Reset()
		return
	}

	if err := s.Scope().ReserveMemory(maxMessageSize, network.ReservationPriorityAlways); err != nil {
		logrus.Debugf("error reserving memory for stream: %s", err)
		s.Reset()
		return
	}
...
func (r *Relay) handleReserve(s network.Stream) pbv2.Status {
	...
	if isRelayAddr(a) {
		logrus.Debugf("refusing relay reservation for %s; reservation attempt over relay connection")
		r.handleError(s, pbv2.Status_PERMISSION_DENIED)
		return pbv2.Status_PERMISSION_DENIED
	}

	if r.acl != nil && !r.acl.AllowReserve(p, a) {
		logrus.Debugf("refusing relay reservation for %s; permission denied", p)
		r.handleError(s, pbv2.Status_PERMISSION_DENIED)
		return pbv2.Status_PERMISSION_DENIED
	}
	...
	if r.closed {
		r.mx.Unlock()
		logrus.Debugf("refusing relay reservation for %s; relay closed", p)
		r.handleError(s, pbv2.Status_PERMISSION_DENIED)
		return pbv2.Status_PERMISSION_DENIED
	}
	now := time.Now()
	...
	if !exists {
		if err := r.constraints.AddReservation(p, a); err != nil {
			r.mx.Unlock()
			logrus.Debugf("refusing relay reservation for %s; IP constraint violation: %s", p, err)
	...
	logrus.Debugf("reserving relay slot for %s", p)
	...
	if err := r.writeResponse(s, pbv2.Status_OK, r.makeReservationMsg(p, expire), r.makeLimitMsg(p)); err != nil {
		logrus.Debugf("error writing reservation response; retracting reservation for %s", p)
		s.Reset()
...
func (r *Relay) handleConnect(s network.Stream, msg *pbv2.HopMessage) pbv2.Status {
	...
	if err != nil {
		logrus.Debugf("failed to begin relay transaction: %s", err)
	...
	if err := span.ReserveMemory(2*r.rc.BufferSize, network.ReservationPriorityHigh); err != nil {
		logrus.Debugf("error reserving memory for relay: %s", err)
		fail(pbv2.Status_RESOURCE_LIMIT_EXCEEDED)
		return pbv2.Status_RESOURCE_LIMIT_EXCEEDED
	}

	if isRelayAddr(a) {
		logrus.Debugf("refusing connection from %s; connection attempt over relay connection")
	...
	if r.acl != nil && !r.acl.AllowConnect(src, s.Conn().RemoteMultiaddr(), dest.ID) {
		logrus.Debugf("refusing connection from %s to %s; permission denied", src, dest.ID)
		fail(pbv2.Status_PERMISSION_DENIED)
		return pbv2.Status_PERMISSION_DENIED
	}
	...
	if !rsvp {
		r.mx.Unlock()
		logrus.Debugf("refusing connection from %s to %s; no reservation", src, dest.ID)
	...
	if srcConns >= r.rc.MaxCircuits {
		r.mx.Unlock()
		logrus.Debugf("refusing connection from %s to %s; too many connections from %s", src, dest.ID, src)
		fail(pbv2.Status_RESOURCE_LIMIT_EXCEEDED)
		return pbv2.Status_RESOURCE_LIMIT_EXCEEDED
	}
	...
	if destConns >= r.rc.MaxCircuits {
		r.mx.Unlock()
		logrus.Debugf("refusing connection from %s to %s; too many connecitons to %s", src, dest.ID, dest.ID)
		fail(pbv2.Status_RESOURCE_LIMIT_EXCEEDED)
		return pbv2.Status_RESOURCE_LIMIT_EXCEEDED
	}
	...
	if err != nil {
		logrus.Debugf("error opening relay stream to %s: %s", dest.ID, err)
		cleanup()
	...
	if err != nil {
		logrus.Debugf("error writing stop handshake")
	...
	if err != nil {
		logrus.Debugf("error reading stop response: %s", err.Error())
		fail(pbv2.Status_CONNECTION_FAILED)
		return pbv2.Status_CONNECTION_FAILED
	}

	if t := stopmsg.GetType(); t != pbv2.StopMessage_STATUS {
		logrus.Debugf("unexpected stop response; not a status message (%d)", t)
		fail(pbv2.Status_CONNECTION_FAILED)
		return pbv2.Status_CONNECTION_FAILED
	}

	if status := stopmsg.GetStatus(); status != pbv2.Status_OK {
		logrus.Debugf("relay stop failure: %d", status)
		fail(pbv2.Status_CONNECTION_FAILED)
		return pbv2.Status_CONNECTION_FAILED
	}
	...
	if err != nil {
		logrus.Debugf("error writing relay response: %s", err)
		bs.Reset()
		s.Reset()
		cleanup()
		return pbv2.Status_CONNECTION_FAILED
	...
	logrus.Infof("relaying connection from %s to %s", src, dest.ID)
...
func (r *Relay) relayLimited(src, dest network.Stream, srcID, destID peer.ID, limit int64, done func()) {
	...
	if err != nil {
		logrus.Debugf("relay copy error: %s", err)
		// Reset both.
		src.Reset()
		dest.Reset()
	} else {
	...
	logrus.Debugf("relayed %d bytes from %s to %s", count, srcID, destID)
}
...
func (r *Relay) handleError(s network.Stream, status pbv2.Status) {
	logrus.Debugf("relay error: %s (%d)", pbv2.Status_name[int32(status)], status)
...
func (r *Relay) makeReservationMsg(p peer.ID, expire time.Time) *pbv2.Reservation {
	...
	if err != nil {
		logrus.Errorf("error sealing voucher for %s: %s", p, err)
		return rsvp
	}

	blob, err := envelope.Marshal()
	if err != nil {
		logrus.Errorf("error marshalling voucher for %s: %s", p, err)
		return rsvp
	}
