...
func (c *Client) handleStreamV2(s network.Stream) {
	logrus.Debugf("new relay/v2 stream from: %s", s.Conn().RemotePeer())
	...
	handleError := func(status pbv2.Status) {
		logrus.Debugf("protocol error: %s (%d)", pbv2.Status_name[int32(status)], status)
		err := writeResponse(status)
		if err != nil {
			s.Reset()
			logrus.Debugf("error writing circuit response: %s", err.Error())
		} else {
			s.Close()
		}
	}
	...
	logrus.Debugf("incoming relay connection from: %s", src.ID)
