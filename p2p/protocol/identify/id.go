...
import (
	...
	"github.com/bpfs/dep2p/core/crypto"
	"github.com/bpfs/dep2p/core/event"
	...
	"github.com/bpfs/dep2p/core/record"
	"github.com/bpfs/dep2p/p2p/host/eventbus"
	"github.com/bpfs/dep2p/p2p/protocol/identify/pb"
	"github.com/sirupsen/logrus"

	ma "github.com/multiformats/go-multiaddr"
	manet "github.com/multiformats/go-multiaddr/net"
	msmux "github.com/multiformats/go-multistream"
	"github.com/bpfs/dep2p/util/msgio/pbio"
...
const (
	...
	IDPush = "/dep2p/id/push/1.0.0"
)

const ServiceName = "dep2p.identify"
...
var defaultUserAgent = "github.com/bpfs/dep2p"
...
//
// The idService sends:
//   - Our dep2p Protocol Version
//   - Our dep2p Agent Version
...
func NewIDService(h host.Host, opts ...Option) (*idService, error) {
	...
	if err != nil {
		logrus.Warnf("identify service not emitting peer protocol updates; err: %s", err)
	}
	s.emitters.evtPeerIdentificationCompleted, err = h.EventBus().Emitter(&event.EvtPeerIdentificationCompleted{})
	if err != nil {
		logrus.Warnf("identify service not emitting identification completed events; err: %s", err)
	}
	s.emitters.evtPeerIdentificationFailed, err = h.EventBus().Emitter(&event.EvtPeerIdentificationFailed{})
	if err != nil {
		logrus.Warnf("identify service not emitting identification failed events; err: %s", err)
	}
	return s, nil
}
...
func (ids *idService) loop(ctx context.Context) {
	...
	if err != nil {
		logrus.Errorf("failed to subscribe to events on the bus, err=%s", err)
		return
	}
	defer sub.Close()
...
func (ids *idService) sendPushes(ctx context.Context) {
	...
	for _, c := range conns {
		...
		if e.Sequence >= snapshot.seq {
			logrus.Debug("already sent this snapshot to peer", "peer", c.RemotePeer(), "seq", snapshot.seq)
			continue
		}
		...
		go func(c network.Conn) {
			...
			if err := ids.sendIdentifyResp(str, true); err != nil {
				logrus.Debug("failed to send identify push", "peer", c.RemotePeer(), "error", err)
				return
			}
		}(c)
	}
...
func (ids *idService) IdentifyWait(c network.Conn) <-chan struct{} {
	...
	if !found {
		...
		if c.IsClosed() {
			logrus.Debug("connection not found in identify service", "peer", c.RemotePeer())
			ch := make(chan struct{})
			close(ch)
			return ch
		} else {
	...
	go func() {
		...
		if err := ids.identifyConn(c); err != nil {
			logrus.Warnf("failed to identify %s: %s", c.RemotePeer(), err)
...
func (ids *idService) identifyConn(c network.Conn) error {
	...
	if err != nil {
		logrus.Debug("error opening identify stream", "peer", c.RemotePeer(), "error", err)
		return err
	}
	s.SetDeadline(time.Now().Add(Timeout))

	if err := s.SetProtocol(ID); err != nil {
		logrus.Warnf("error setting identify protocol for stream: %s", err)
		s.Reset()
	}

	// ok give the response to our handler.
	if err := msmux.SelectProtoOrFail(ID, s); err != nil {
		logrus.Info("failed negotiate identify protocol with peer", "peer", c.RemotePeer(), "error", err)
		s.Reset()
		return err
	}
...
func (ids *idService) sendIdentifyResp(s network.Stream, isPush bool) error {
	...
	logrus.Debug("sending snapshot", "seq", snapshot.seq, "protocols", snapshot.protocols, "addrs", snapshot.addrs)

	mes := ids.createBaseIdentifyResponse(s.Conn(), &snapshot)
	mes.SignedPeerRecord = ids.getSignedRecord(&snapshot)

	logrus.Debugf("%s sending message to %s %s", ID, s.Conn().RemotePeer(), s.Conn().RemoteMultiaddr())
...
func (ids *idService) handleIdentifyResponse(s network.Stream, isPush bool) error {
	if err := s.Scope().SetService(ServiceName); err != nil {
		logrus.Warnf("error attaching stream to identify service: %s", err)
		s.Reset()
		return err
	}

	if err := s.Scope().ReserveMemory(signedIDSize, network.ReservationPriorityAlways); err != nil {
		logrus.Warnf("error reserving memory for identify stream: %s", err)
		s.Reset()
		return err
	}
	...
	if err := readAllIDMessages(r, mes); err != nil {
		logrus.Warn("error reading identify message: ", err)
		s.Reset()
		return err
	}
	...
	logrus.Debugf("%s received message from %s %s", s.Protocol(), c.RemotePeer(), c.RemoteMultiaddr())
...
func (ids *idService) updateSnapshot() (updated bool) {
	...
	logrus.Debug("updating snapshot", "seq", snapshot.seq, "addrs", snapshot.addrs)
	return true
}
...
func (ids *idService) createBaseIdentifyResponse(conn network.Conn, snapshot *identifySnapshot) *pb.Identify {
	...
	if ownKey == nil {
		...
		if ids.Host.Peerstore().PrivKey(ids.Host.ID()) != nil {
			...
			logrus.Errorf("did not have own public key in Peerstore")
		}
		...
		if kb, err := crypto.MarshalPublicKey(ownKey); err != nil {
			logrus.Errorf("failed to convert key to bytes")
		} else {
			mes.PublicKey = kb
		}
	}
...
func (ids *idService) getSignedRecord(snapshot *identifySnapshot) []byte {
	...
	if err != nil {
		logrus.Error("failed to marshal signed record", "err", err)
		return nil
	}
...
func (ids *idService) consumeMessage(mes *pb.Identify, c network.Conn, isPush bool) {
	...
	for _, addr := range laddrs {
		...
		if err != nil {
			logrus.Debugf("%s failed to parse multiaddr from %s %s", ID,
				p, c.RemoteMultiaddr())
			continue
		}
	...
	if err != nil {
		logrus.Errorf("error getting peer record from Identify message: %v", err)
	}
	...
	if signedPeerRecord != nil {
		...
		if err != nil {
			logrus.Debugf("failed to consume signed peer record: %s", err)
		} else {
			addrs = signedAddrs
		}
	} else {
	...
	logrus.Debugf("%s received listen addrs for %s: %s", c.LocalPeer(), c.RemotePeer(), lmaddrs)
...
func (ids *idService) consumeReceivedPubKey(c network.Conn, kb []byte) {
	...
	if kb == nil {
		logrus.Debugf("%s did not receive public key for remote peer: %s", lp, rp)
		return
	}

	newKey, err := crypto.UnmarshalPublicKey(kb)
	if err != nil {
		logrus.Warnf("%s cannot unmarshal key from remote peer: %s, %s", lp, rp, err)
		return
	}
	...
	if err != nil {
		logrus.Debugf("%s cannot get peer.ID from key of remote peer: %s, %s", lp, rp, err)
		return
	}
	...
	if np != rp {
		...
		if rp == "" && np != "" {
			...
			if err != nil {
				logrus.Debugf("%s could not add key for %s to peerstore: %s", lp, rp, err)
			}

		} else {
			// we have a local peer.ID and it does not match the sent key... error.
			logrus.Errorf("%s received key for remote peer %s mismatch: %s", lp, rp, np)
		}
		return
	}
	...
	logrus.Errorf("%s identify got a different key for: %s", lp, rp)

	// okay... does ours NOT match the remote peer.ID?
	cp, err := peer.IDFromPublicKey(currKey)
	if err != nil {
		logrus.Errorf("%s cannot get peer.ID from local key of remote peer: %s, %s", lp, rp, err)
		return
	}
	if cp != rp {
		logrus.Errorf("%s local key for remote peer %s yields different peer.ID: %s", lp, rp, cp)
		return
	}

	// okay... curr key DOES NOT match new key. both match peer.ID. wat?
	logrus.Errorf("%s local key and received key for %s do not match, but match peer.ID", lp, rp)
}
...
func (ids *idService) consumeObservedAddress(observed []byte, c network.Conn) {
	...
	if err != nil {
		logrus.Debugf("error parsing received observed addr for %s: %s", c, err)
		return
	}
