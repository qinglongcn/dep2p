...
func (hp *holePuncher) directConnect(rp peer.ID) error {
	...
	for _, a := range hp.host.Peerstore().Addrs(rp) {
		if manet.IsPublicAddr(a) && !isRelayAddress(a) {
			...
			logrus.Debug("direct connection to peer successful, no need for a hole punch", "peer", rp)
			return nil
		}
	}

	logrus.Debug("got inbound proxy conn", "peer", rp)

	// hole punch
	for i := 1; i <= maxRetries; i++ {
		addrs, obsAddrs, rtt, err := hp.initiateHolePunch(rp)
		if err != nil {
			logrus.Debug("hole punching failed", "peer", rp, "error", err)
			hp.tracer.ProtocolError(rp, err)
			return err
		}
		synTime := rtt / 2
		logrus.Debugf("peer RTT is %s; starting hole punch in %s", rtt, synTime)
		...
		case start := <-timer.C:
			...
			if err == nil {
				logrus.Debug("hole punching with successful", "peer", rp, "time", dt)
