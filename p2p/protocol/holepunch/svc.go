...
import (
	...
	"github.com/bpfs/dep2p/p2p/protocol/holepunch/pb"
...
// Protocol is the dep2p protocol for Hole Punching.
...
func (s *Service) watchForPublicAddr() {
	...
	logrus.Debug("waiting until we have at least one public address", "peer", s.host.ID())
	...
	for {
		if containsPublicAddr(s.ids.OwnObservedAddrs()) {
			logrus.Debug("Host now has a public address. Starting holepunch protocol.")
	...
	if err != nil {
		logrus.Debugf("failed to subscripe to Reachability event: %s", err)
		return
	}
	defer sub.Close()
	for {
...
func (s *Service) incomingHolePunch(str network.Stream) (rtt time.Duration, remoteAddrs []ma.Multiaddr, ownAddrs []ma.Multiaddr, err error) {
	...
	logrus.Debug("received hole punch request", "peer", str.Conn().RemotePeer(), "addrs", obsDial)
	if len(obsDial) == 0 {
...
func (s *Service) handleNewStream(str network.Stream) {
	...
	if err := str.Scope().SetService(ServiceName); err != nil {
		logrus.Debugf("error attaching stream to holepunch service: %s", err)
		str.Reset()
		return
	}
	...
	if err != nil {
		...
		logrus.Debug("error handling holepunching stream from", "peer", rp, "error", err)
		str.Reset()
		return
	}
	str.Close()
	...
	logrus.Debug("starting hole punch", "peer", rp)
	start := time.Now()
