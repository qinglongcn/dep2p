...
func (p *PingService) PingHandler(s network.Stream) {
	if err := s.Scope().SetService(ServiceName); err != nil {
		logrus.Debugf("error attaching stream to ping service: %s", err)
		s.Reset()
		return
	}

	if err := s.Scope().ReserveMemory(PingSize, network.ReservationPriorityAlways); err != nil {
		logrus.Debugf("error reserving memory for ping stream: %s", err)
		s.Reset()
		return
	}
	...
	go func() {
		select {
		case <-timer.C:
			logrus.Debug("ping timeout")
		case err, ok := <-errCh:
			if ok {
				logrus.Debug(err)
			} else {
				logrus.Error("ping loop failed without error")
			}
		}
		s.Close()
	}()
...
func Ping(ctx context.Context, h host.Host, p peer.ID) <-chan Result {
	...
	if _, err := rand.Read(b); err != nil {
		logrus.Errorf("failed to get cryptographic random: %s", err)
		s.Reset()
		return pingError(err)
	}
