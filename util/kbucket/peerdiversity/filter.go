...
import (
	...
	"github.com/bpfs/dep2p/util/cidranger"
...
func (f *Filter) TryAdd(p peer.ID) bool {
	...
	if len(addrs) == 0 {
		logrus.Debug("no addresses found for peer", "appKey", f.logKey, "peer", p)
		return false
	}
	...
	for _, a := range addrs {
		ip, err := manet.ToIP(a)
		if err != nil {
			logrus.Error("failed to parse IP from multiaddr", "appKey", f.logKey,
		...
		if err != nil {
			logrus.Error("failed to find Group Key", "appKey", f.logKey, "ip", ip.String(), "peer", p,
				"err", err)
			return false
		}
		if len(key) == 0 {
			logrus.Error("group key is empty", "appKey", f.logKey, "ip", ip.String(), "peer", p)
			return false
		}
...
func (f *Filter) ipGroupKey(ip net.IP) (PeerIPGroupKey, error) {
	...
	case nil:
		...
		if len(s) == 0 {
			logrus.Debug("ASN not known", "appKey", f.logKey, "ip", ip)
