...
func getValueOrErrorParallel[T any](
	...
	fwg := jsync.NewFWaitGroup(func() {
		close(outCh)
		close(errCh)
		logrus.Debug("getValueOrErrorParallel: finished executing all routers ", len(routers))
	}, uint64(len(routers)))
	for _, r := range routers {
		go func(r *ParallelRouter) {
			defer fwg.Done()
			logrus.Debug("getValueOrErrorParallel: starting execution for router ", r.Router,
			...
			case <-tim.C:
				...
				if err != nil {
					if r.IgnoreError || errors.Is(err, routing.ErrNotFound) {
						logrus.Debug("getValueOrErrorParallel: not found or ignorable error for router ", r.Router,
							" with timeout ", r.Timeout,
							" and ignore errors ", r.IgnoreError,
						)
						return
					}
					logrus.Debug("getValueOrErrorParallel: error calling router function for router ", r.Router,
						" with timeout ", r.Timeout,
						" and ignore errors ", r.IgnoreError,
						" with error ", err,
					)
				...
				if empty {
					logrus.Debug("getValueOrErrorParallel: empty flag for router ", r.Router,
						" with timeout ", r.Timeout,
						" and ignore errors ", r.IgnoreError,
					)
					return
	...
	case out, ok := <-outCh:
		...
		logrus.Debug("getValueOrErrorParallel: value returned by channel")
	...
	case err, ok := <-errCh:
		...
		logrus.Debug("getValueOrErrorParallel: error returned by channel:", err)

		return value, err
	case <-ctx.Done():
		err := ctx.Err()
		logrus.Debug("getValueOrErrorParallel: error on context done:", err)
		return value, err
	}
}
...
func executeParallel(
	...
	for _, r := range routers {
		go func(r *ParallelRouter, ctx context.Context) {
			...
			if err := func() error {
				logrus.Debug("executeParallel: starting execution for router ", r.Router,
					" with timeout ", r.Timeout,
					" and ignore errors ", r.IgnoreError,
				)
				tim := time.NewTimer(r.ExecuteAfter)
				...
				case <-ctx.Done():
					if !r.IgnoreError {
						logrus.Debug("executeParallel: stopping execution on router on context done for router ", r.Router,
							" with timeout ", r.Timeout,
							" and ignore errors ", r.IgnoreError,
						)
						return ctx.Err()
				...
				case <-tim.C:
					...
					logrus.Debug("executeParallel: calling router function for router ", r.Router,
						" with timeout ", r.Timeout,
						" and ignore errors ", r.IgnoreError,
					)
					if err := f(ctx, r.Router); err != nil && !r.IgnoreError {
						logrus.Debug("executeParallel: error calling router function for router ", r.Router,
							" with timeout ", r.Timeout,
							" and ignore errors ", r.IgnoreError,
							" with error ", err,
						)
	...
	if errOut != nil {
		logrus.Debug("executeParallel: finished executing all routers with error: ", errOut)
	}
