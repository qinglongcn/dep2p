...
import (
	...
	ipns_pb "github.com/bpfs/dep2p/util/ipns/pb"
	record "github.com/bpfs/dep2p/util/record"
...
//
// [Record Verification]: https://specs.dep2p.tech/ipns/ipns-record/#record-verification
...
func (v Validator) Validate(key string, value []byte) error {
	...
	if err != nil {
		logrus.Debugf("failed to parse ipns routing key %q into name", key)
...
func (v Validator) getPublicKey(r *Record, name Name) (ic.PubKey, error) {
	...
	if v.KeyBook == nil {
		logrus.Debugf("public key with hash %q not found in IPNS record and no peer store provided", name.Peer())
		return nil, ErrPublicKeyNotFound
	}

	pk := v.KeyBook.PubKey(name.Peer())
	if pk == nil {
		logrus.Debugf("public key with hash %q not found in peer store", name.Peer())
		return nil, ErrPublicKeyNotFound
	}
