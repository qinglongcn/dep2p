...
// distributed information systems. CIDs are used in the DeP2P
// (https://dep2p.io) project ecosystem.
//
...
const (
	...
	DeP2PKey  = 0x72   // https://github.com/dep2p/specs/blob/master/peer-ids/peer-ids.md#peer-ids
...
// They exist to allow DeP2P to work with Cids while keeping
// compatibility with the plain-multihash format used used in DeP2P.
...
func Parse(v interface{}) (Cid, error) {
	switch v2 := v.(type) {
	case string:
		if strings.Contains(v2, "/dep2p/") {
			return Decode(strings.Split(v2, "/dep2p/")[1])
		}
		return Decode(v2)
	case []byte:
		return Cast(v2)
