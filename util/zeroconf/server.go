...
func newServer(ifaces []net.Interface, opts serverOpts) (*Server, error) {
	...
	if err4 != nil {
		logrus.Printf("[zeroconf] no suitable IPv4 interface: %s", err4.Error())
	}
	ipv6conn, err6 := joinUdp6Multicast(ifaces)
	if err6 != nil {
		logrus.Printf("[zeroconf] no suitable IPv6 interface: %s", err6.Error())
	}
...
func (s *Server) Shutdown() {
	...
	if err := s.unregister(); err != nil {
		logrus.Printf("failed to unregister: %s", err)
	}
...
func (s *Server) parsePacket(packet []byte, ifIndex int, from net.Addr) error {
	...
	if err := msg.Unpack(packet); err != nil {
		// logrus.Printf("[ERR] zeroconf: Failed to unpack packet: %v", err)
		return err
	}
...
func (s *Server) handleQuery(query *dns.Msg, ifIndex int, from net.Addr) error {
	...
	for _, q := range query.Question {
		...
		if err = s.handleQuestion(q, &resp, query, ifIndex); err != nil {
			// logrus.Printf("[ERR] zeroconf: failed to handle question %v: %v", q, err)
			continue
		}
...
func isKnownAnswer(resp *dns.Msg, query *dns.Msg) bool {
	...
	for _, known := range query.Answer {
		...
		if ptr.Ptr == answer.Ptr && hdr.Ttl >= answer.Hdr.Ttl/2 {
			// logrus.Printf("skipping known answer: %v", ptr)
			return true
		}
	}
...
func (s *Server) probe() {
	...
	for i := 0; i < 3; i++ {
		if err := s.multicastResponse(q, 0); err != nil {
			logrus.Println("[ERR] zeroconf: failed to send probe:", err.Error())
		}
	...
	for i := 0; i < multicastRepetitions; i++ {
		for _, intf := range s.ifaces {
			...
			if err := s.multicastResponse(resp, intf.Index); err != nil {
				logrus.Println("[ERR] zeroconf: failed to send announcement:", err.Error())
			}
		}
		timer.Reset(timeout)
		select {
		...
		if ifIndex != 0 {
			...
			default:
				...
				if err := s.ipv4conn.SetMulticastInterface(iface); err != nil {
					logrus.Printf("[WARN] mdns: Failed to set multicast interface: %v", err)
				}
			}
