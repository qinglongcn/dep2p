...
// capabilities required by dep2p: stream multiplexing, encryption and
// peer authentication.
//
...
//
// If a transport implements `io.Closer` (optional), dep2p will call `Close` on
// shutdown. NOTE: `Dial` and `Listen` may be called after or concurrently with
// `Close`.
//
// For a conceptual overview, see https://docs.dep2p.io/concepts/transport/
